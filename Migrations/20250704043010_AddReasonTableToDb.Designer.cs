// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using iDss.X.Data;

#nullable disable

namespace iDss.X.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20250704043010_AddReasonTableToDb")]
    partial class AddReasonTableToDb
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.2")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("iDss.X.Models.AWBInventory", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<int>("flagprint")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("userlock")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("id");

                    b.HasIndex("awb")
                        .IsUnique();

                    b.HasIndex("branchid");

                    b.ToTable("mdt_awbinventory");
                });

            modelBuilder.Entity("iDss.X.Models.Account", b =>
                {
                    b.Property<string>("acctno")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("acctname")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime?>("agreedate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("agreeexpire")
                        .HasColumnType("datetime2");

                    b.Property<string>("bankacctname")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("bankacctno")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("bankcode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("cif")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("costcenter")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<decimal?>("creditlimit")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("creditperiod")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<decimal?>("discrates")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("feecod")
                        .HasColumnType("decimal(10,2)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("frp")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int?>("iscod")
                        .HasColumnType("int");

                    b.Property<int?>("isintl")
                        .HasColumnType("int");

                    b.Property<int?>("isnl")
                        .HasColumnType("int");

                    b.Property<int?>("isrev")
                        .HasColumnType("int");

                    b.Property<int?>("istrace")
                        .HasColumnType("int");

                    b.Property<int?>("isvat")
                        .HasColumnType("int");

                    b.Property<string>("lob")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal?>("mgmtfee")
                        .HasColumnType("decimal(10,2)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal?>("ppn")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("stampcosttype")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("termofpayment")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("vattype")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("acctno");

                    b.HasIndex("branchid");

                    b.HasIndex("cif");

                    b.ToTable("mdt_account");
                });

            modelBuilder.Entity("iDss.X.Models.AccountAddr", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("Relational:JsonPropertyName", "id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("acctno")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr3")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addrtype")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<string>("email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("latitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("longitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("picname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("postcode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("id");

                    b.HasIndex("acctno");

                    b.HasIndex("distid");

                    b.ToTable("mdt_accountaddr");
                });

            modelBuilder.Entity("iDss.X.Models.AccountCro", b =>
                {
                    b.Property<string>("acctno")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("crocode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.HasKey("acctno", "crocode");

                    b.HasIndex("crocode");

                    b.ToTable("mdt_accountcro");
                });

            modelBuilder.Entity("iDss.X.Models.Agent", b =>
                {
                    b.Property<int>("agentid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("agentid"));

                    b.Property<string>("addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr3")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("agentcode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("agentname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("citycode")
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("comission")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("email")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("latitude")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("longitude")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("phonealt")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("picname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("picno")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("postcode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("villid")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("agentid");

                    b.HasIndex("branchid");

                    b.HasIndex("villid");

                    b.ToTable("mdt_agent");
                });

            modelBuilder.Entity("iDss.X.Models.AppMenu", b =>
                {
                    b.Property<string>("menuid")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("icon")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("menuname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("menusort")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("moduleid")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("parentid")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("path")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("properties")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("menuid");

                    b.HasIndex("moduleid");

                    b.ToTable("app_menu");

                    b.HasData(
                        new
                        {
                            menuid = "1.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-fingerprint",
                            menuname = "Identity",
                            menusort = 1,
                            moduleid = "101",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "1.1.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-user",
                            menuname = "User Data",
                            menusort = 1,
                            moduleid = "101",
                            parentid = "1.1",
                            path = "/admin/user"
                        },
                        new
                        {
                            menuid = "1.1.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-user-shield",
                            menuname = "Role Data",
                            menusort = 2,
                            moduleid = "101",
                            parentid = "1.1",
                            path = "/admin/role"
                        },
                        new
                        {
                            menuid = "1.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-wrench",
                            menuname = "Configuration",
                            menusort = 2,
                            moduleid = "101",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "1.2.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-shoe-prints",
                            menuname = "Audittrail",
                            menusort = 1,
                            moduleid = "101",
                            parentid = "1.2",
                            path = "/admin/audittrail"
                        },
                        new
                        {
                            menuid = "1.2.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-bell",
                            menuname = "Preparing Sign-out",
                            menusort = 2,
                            moduleid = "101",
                            parentid = "1.2",
                            path = "/admin/preparingsignout"
                        },
                        new
                        {
                            menuid = "2.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-building",
                            menuname = "Branch & Agent",
                            menusort = 1,
                            moduleid = "102",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "2.1.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-building-flag",
                            menuname = "NCS Branch Office",
                            menusort = 1,
                            moduleid = "102",
                            parentid = "2.1",
                            path = "/master/branch"
                        },
                        new
                        {
                            menuid = "2.1.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-shop",
                            menuname = "NCS Counter",
                            menusort = 2,
                            moduleid = "102",
                            parentid = "2.1",
                            path = "/master/counter"
                        },
                        new
                        {
                            menuid = "2.1.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-store",
                            menuname = "NCS Agent",
                            menusort = 3,
                            moduleid = "102",
                            parentid = "2.1",
                            path = "/master/agent"
                        },
                        new
                        {
                            menuid = "2.1.4",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid  fa-house-laptop",
                            menuname = "NCS Agent Otonom",
                            menusort = 4,
                            moduleid = "102",
                            parentid = "2.1",
                            path = "/master/agentotonom"
                        },
                        new
                        {
                            menuid = "2.1.5",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-person-booth",
                            menuname = "Wholesaler",
                            menusort = 5,
                            moduleid = "102",
                            parentid = "2.1",
                            path = "/master/wholesaler"
                        },
                        new
                        {
                            menuid = "2.1.6",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-envelopes-bulk",
                            menuname = "Branch Corporate",
                            menusort = 6,
                            moduleid = "102",
                            parentid = "2.1",
                            path = "/master/branchcorporate"
                        },
                        new
                        {
                            menuid = "2.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-warehouse",
                            menuname = "Operational",
                            menusort = 2,
                            moduleid = "102",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "2.2.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-people-line",
                            menuname = "Courier Group",
                            menusort = 1,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/couriergroup"
                        },
                        new
                        {
                            menuid = "2.2.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-motorcycle",
                            menuname = "Courier Code",
                            menusort = 2,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/courier"
                        },
                        new
                        {
                            menuid = "2.2.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-road",
                            menuname = "Courier Route",
                            menusort = 3,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/courierroute"
                        },
                        new
                        {
                            menuid = "2.2.4",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-location-dot",
                            menuname = "Checkpoint Code",
                            menusort = 4,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/checkpointcode"
                        },
                        new
                        {
                            menuid = "2.2.5",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-building-circle-arrow-right",
                            menuname = "HUB Code",
                            menusort = 5,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/hubcode"
                        },
                        new
                        {
                            menuid = "2.2.6",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-people-arrows",
                            menuname = "Relation Code",
                            menusort = 6,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/relation"
                        },
                        new
                        {
                            menuid = "2.2.8",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-bell-concierge",
                            menuname = "Service Code",
                            menusort = 8,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/service"
                        },
                        new
                        {
                            menuid = "2.2.9",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-boxes-packing",
                            menuname = "Packing",
                            menusort = 9,
                            moduleid = "102",
                            parentid = "2.2",
                            path = ""
                        },
                        new
                        {
                            menuid = "2.2.9.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-boxes-packing",
                            menuname = "Packing Type",
                            menusort = 1,
                            moduleid = "102",
                            parentid = "2.2.9",
                            path = "/master/packingtype"
                        },
                        new
                        {
                            menuid = "2.2.9.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-boxes-packing",
                            menuname = "Packing Size",
                            menusort = 2,
                            moduleid = "102",
                            parentid = "2.2.9",
                            path = "/master/packingsize"
                        },
                        new
                        {
                            menuid = "2.2.9.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-boxes-packing",
                            menuname = "Packing Price",
                            menusort = 3,
                            moduleid = "102",
                            parentid = "2.2.9",
                            path = "/master/packingprice"
                        },
                        new
                        {
                            menuid = "2.2.10",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-filter-circle-dollar",
                            menuname = "Cost Component",
                            menusort = 10,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/costcomponent"
                        },
                        new
                        {
                            menuid = "2.2.11",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-circle-dollar-to-slot",
                            menuname = "Payment Gateway",
                            menusort = 11,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/paymentgateway"
                        },
                        new
                        {
                            menuid = "2.2.12",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-road-circle-exclamation",
                            menuname = "Reason for Undelivered",
                            menusort = 11,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/reasonun"
                        },
                        new
                        {
                            menuid = "2.2.13",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-road-circle-exclamation",
                            menuname = "Reason Pickup",
                            menusort = 12,
                            moduleid = "102",
                            parentid = "2.2",
                            path = "/master/reasonpickup"
                        },
                        new
                        {
                            menuid = "2.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-user",
                            menuname = "Client",
                            menusort = 3,
                            moduleid = "102",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "2.3.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-users-between-lines",
                            menuname = "CIF Data Client",
                            menusort = 1,
                            moduleid = "102",
                            parentid = "2.3",
                            path = "/master/cif"
                        },
                        new
                        {
                            menuid = "2.3.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-user-tag",
                            menuname = "Account Data Client",
                            menusort = 2,
                            moduleid = "102",
                            parentid = "2.3",
                            path = "/master/account"
                        },
                        new
                        {
                            menuid = "2.3.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-industry",
                            menuname = "Industry Code",
                            menusort = 3,
                            moduleid = "102",
                            parentid = "2.3",
                            path = "/master/industry"
                        },
                        new
                        {
                            menuid = "2.4",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-handshake",
                            menuname = "Sales",
                            menusort = 4,
                            moduleid = "102",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "2.4.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-person-arrow-up-from-line",
                            menuname = "CRO",
                            menusort = 1,
                            moduleid = "102",
                            parentid = "2.4",
                            path = "/master/cro"
                        },
                        new
                        {
                            menuid = "2.5",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-map-location-dot",
                            menuname = "Area",
                            menusort = 5,
                            moduleid = "102",
                            parentid = "",
                            path = ""
                        },
                        new
                        {
                            menuid = "2.5.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-mountain-city",
                            menuname = "Country",
                            menusort = 1,
                            moduleid = "102",
                            parentid = "2.5",
                            path = "/master/country"
                        },
                        new
                        {
                            menuid = "2.5.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-mountain-city",
                            menuname = "Province",
                            menusort = 2,
                            moduleid = "102",
                            parentid = "2.5",
                            path = "/master/province"
                        },
                        new
                        {
                            menuid = "2.5.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-tree-city",
                            menuname = "City",
                            menusort = 3,
                            moduleid = "102",
                            parentid = "2.5",
                            path = "/master/city"
                        },
                        new
                        {
                            menuid = "2.5.4",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-tree-city",
                            menuname = "City International",
                            menusort = 4,
                            moduleid = "102",
                            parentid = "2.5",
                            path = "/master/cityintl"
                        },
                        new
                        {
                            menuid = "2.5.5",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-bridge-circle-exclamation",
                            menuname = "District",
                            menusort = 5,
                            moduleid = "102",
                            parentid = "2.5",
                            path = "/master/district"
                        },
                        new
                        {
                            menuid = "2.5.6",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-diamond",
                            menuname = "Village",
                            menusort = 6,
                            moduleid = "102",
                            parentid = "2.5",
                            path = "/master/village"
                        },
                        new
                        {
                            menuid = "2.5.7",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-diamond",
                            menuname = "Postal Code",
                            menusort = 7,
                            moduleid = "102",
                            parentid = "2.5",
                            path = "/master/postcode"
                        },
                        new
                        {
                            menuid = "2.5.8",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-diamond",
                            menuname = "Zone",
                            menusort = 8,
                            moduleid = "102",
                            parentid = "2.5",
                            path = "/master/zone"
                        },
                        new
                        {
                            menuid = "6.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-truck-ramp-box",
                            menuname = "Pickup Entry",
                            menusort = 1,
                            moduleid = "106",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "6.1.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-calendar-plus",
                            menuname = "Create Pickup Schedule",
                            menusort = 1,
                            moduleid = "106",
                            parentid = "6.1",
                            path = "/pickup/entrypickupschedule"
                        },
                        new
                        {
                            menuid = "6.1.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-phone-volume",
                            menuname = "Entry Pickup On-Call",
                            menusort = 2,
                            moduleid = "106",
                            parentid = "6.1",
                            path = "/pickup/entrypickuponcall"
                        },
                        new
                        {
                            menuid = "6.1.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-user-pen",
                            menuname = "Entry Pickup Client",
                            menusort = 3,
                            moduleid = "106",
                            parentid = "6.1",
                            path = "/pickup/entrypickupclient"
                        },
                        new
                        {
                            menuid = "6.1.4",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-diamond",
                            menuname = "Entry Pickup Custom",
                            menusort = 4,
                            moduleid = "106",
                            parentid = "6.1"
                        },
                        new
                        {
                            menuid = "6.1.4.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-diamond",
                            menuname = "Entry Pickup Danamon (Pickup)",
                            menusort = 1,
                            moduleid = "106",
                            parentid = "6.1.4",
                            path = "/pickup/pickupdanamon"
                        },
                        new
                        {
                            menuid = "6.1.4.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-diamond",
                            menuname = "Entry Pickup Danamon (Retrieval)",
                            menusort = 2,
                            moduleid = "106",
                            parentid = "6.1.4",
                            path = "/pickup/pickupdanamon"
                        },
                        new
                        {
                            menuid = "6.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-cube",
                            menuname = "Pickup Status",
                            menusort = 2,
                            moduleid = "106",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "6.2.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-marker",
                            menuname = "Assign Branch in Charge",
                            menusort = 1,
                            moduleid = "106",
                            parentid = "6.2",
                            path = "/pickup/assignpickup"
                        },
                        new
                        {
                            menuid = "6.2.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-marker",
                            menuname = "Assign Courier",
                            menusort = 2,
                            moduleid = "106",
                            parentid = "6.2",
                            path = "/pickup/assigncourier"
                        },
                        new
                        {
                            menuid = "6.2.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-marker",
                            menuname = "Reschedule Pickup",
                            menusort = 3,
                            moduleid = "106",
                            parentid = "6.2",
                            path = "/pickup/reschedulepickup"
                        },
                        new
                        {
                            menuid = "6.2.4",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-marker",
                            menuname = "Dispatch to Courier",
                            menusort = 4,
                            moduleid = "106",
                            parentid = "6.2",
                            path = "/pickup/dispatchtocourier"
                        },
                        new
                        {
                            menuid = "6.2.5",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-marker",
                            menuname = "Success Pikup",
                            menusort = 5,
                            moduleid = "106",
                            parentid = "6.2",
                            path = "/pickup/successpickup"
                        },
                        new
                        {
                            menuid = "6.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-cube",
                            menuname = "Monitoring Pickup",
                            menusort = 3,
                            moduleid = "106",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "6.3.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-diamond",
                            menuname = "View Timeline Status",
                            menusort = 1,
                            moduleid = "106",
                            parentid = "6.3",
                            path = "/pickup/viewstatuspickup"
                        },
                        new
                        {
                            menuid = "7.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-cube",
                            menuname = "Pra-Delivery",
                            menusort = 1,
                            moduleid = "107",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "7.1.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-list-ol",
                            menuname = "Airwaybill Inventory",
                            menusort = 1,
                            moduleid = "107",
                            parentid = "7.1",
                            path = "/ocl/awbinventory"
                        },
                        new
                        {
                            menuid = "7.1.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-filter",
                            menuname = "Pra-Upload",
                            menusort = 2,
                            moduleid = "107",
                            parentid = "7.1",
                            path = "/ocl/praupload"
                        },
                        new
                        {
                            menuid = "7.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-floppy-disk",
                            menuname = "Corporate Data Entry",
                            menusort = 2,
                            moduleid = "107",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "7.2.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Data Primary",
                            menusort = 1,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/entrydataprimary"
                        },
                        new
                        {
                            menuid = "7.2.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Data National",
                            menusort = 2,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/entrydatanational"
                        },
                        new
                        {
                            menuid = "7.2.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Data Internal",
                            menusort = 3,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/entrydatainternal"
                        },
                        new
                        {
                            menuid = "7.2.4",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Data COD",
                            menusort = 4,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/entrydatacod"
                        },
                        new
                        {
                            menuid = "7.2.5",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Data Preprinted",
                            menusort = 5,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/entrydatapriprinted"
                        },
                        new
                        {
                            menuid = "7.2.6",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Data trucking",
                            menusort = 6,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/entrydatatrucking"
                        },
                        new
                        {
                            menuid = "7.2.7",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Data API",
                            menusort = 7,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/entrydataapi"
                        },
                        new
                        {
                            menuid = "7.2.8",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-file-excel",
                            menuname = "Upload Data Primary",
                            menusort = 8,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/uploaddataprimary"
                        },
                        new
                        {
                            menuid = "7.2.9",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-file-excel",
                            menuname = "Upload Data COD",
                            menusort = 9,
                            moduleid = "107",
                            parentid = "7.2",
                            path = "/ocl/uploaddatacod"
                        },
                        new
                        {
                            menuid = "7.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-floppy-disk",
                            menuname = "Retail Data Entry",
                            menusort = 3,
                            moduleid = "107",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "7.3.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Retail Regular",
                            menusort = 1,
                            moduleid = "107",
                            parentid = "7.3",
                            path = "/ocl/entryretailprimary"
                        },
                        new
                        {
                            menuid = "7.3.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Retail NFD",
                            menusort = 2,
                            moduleid = "107",
                            parentid = "7.3",
                            path = "/ocl/entryretailnfd"
                        },
                        new
                        {
                            menuid = "7.3.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Retail API",
                            menusort = 3,
                            moduleid = "107",
                            parentid = "7.3",
                            path = "/ocl/entryretailapi"
                        },
                        new
                        {
                            menuid = "7.4",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-earth-americas",
                            menuname = "International Data Entry",
                            menusort = 4,
                            moduleid = "107",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "7.4.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-pen",
                            menuname = "Entry Intl Regular",
                            menusort = 1,
                            moduleid = "107",
                            parentid = "7.4",
                            path = "/ocl/entrydataintl"
                        },
                        new
                        {
                            menuid = "7.5",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-tags",
                            menuname = "Shipping Label",
                            menusort = 5,
                            moduleid = "107",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "7.5.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-print",
                            menuname = "Print AWB Primary",
                            menusort = 1,
                            moduleid = "107",
                            parentid = "7.5",
                            path = "/ocl/printawbprimary"
                        },
                        new
                        {
                            menuid = "7.5.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-print",
                            menuname = "Print AWB Internal",
                            menusort = 2,
                            moduleid = "107",
                            parentid = "7.5",
                            path = "/ocl/printawbinternal"
                        },
                        new
                        {
                            menuid = "7.5.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-print",
                            menuname = "Print AWB Intl",
                            menusort = 3,
                            moduleid = "107",
                            parentid = "7.5",
                            path = "/ocl/printawbnational"
                        },
                        new
                        {
                            menuid = "7.6",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-cube",
                            menuname = "Shipment Validation",
                            menusort = 6,
                            moduleid = "107",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "7.6.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-diamond",
                            menuname = "Reconcile AWB",
                            menusort = 1,
                            moduleid = "107",
                            parentid = "7.6",
                            path = "/ocl/shipmentreconcile"
                        },
                        new
                        {
                            menuid = "7.7",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-box-open",
                            menuname = "Bagging",
                            menusort = 7,
                            moduleid = "107",
                            parentid = ""
                        },
                        new
                        {
                            menuid = "7.7.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-box",
                            menuname = "Mother Bag",
                            menusort = 1,
                            moduleid = "107",
                            parentid = "7.7",
                            path = "/ocl/motherbag"
                        },
                        new
                        {
                            menuid = "7.7.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-sack-xmark",
                            menuname = "Baby Bag",
                            menusort = 2,
                            moduleid = "107",
                            parentid = "7.7",
                            path = "/ocl/babybag"
                        },
                        new
                        {
                            menuid = "7.7.3",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-boxes-packing",
                            menuname = "Loose Bag",
                            menusort = 3,
                            moduleid = "107",
                            parentid = "7.7",
                            path = "/ocl/loosebag"
                        },
                        new
                        {
                            menuid = "21.1",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-tag",
                            menuname = "Price Regular",
                            menusort = 1,
                            moduleid = "121",
                            parentid = "",
                            path = "/master/priceregular"
                        },
                        new
                        {
                            menuid = "21.2",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            icon = "fa-solid fa-tag",
                            menuname = "Price Custom",
                            menusort = 2,
                            moduleid = "121",
                            parentid = "",
                            path = "/master/pricecustom"
                        });
                });

            modelBuilder.Entity("iDss.X.Models.AppModule", b =>
                {
                    b.Property<string>("moduleid")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("icon")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("modulectgid")
                        .HasColumnType("int");

                    b.Property<string>("modulename")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int>("modulesort")
                        .HasColumnType("int");

                    b.HasKey("moduleid");

                    b.HasIndex("modulectgid");

                    b.ToTable("app_module");

                    b.HasData(
                        new
                        {
                            moduleid = "101",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Manages system settings, permissions and general platform configurations",
                            flag = 1,
                            icon = "fa-solid fa-user-tie",
                            modulectgid = 1,
                            modulename = "Administrasion",
                            modulesort = 1
                        },
                        new
                        {
                            moduleid = "102",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Stores and manages core data",
                            flag = 1,
                            icon = "fa-solid fa-database",
                            modulectgid = 2,
                            modulename = "Master Data",
                            modulesort = 2
                        },
                        new
                        {
                            moduleid = "103",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Handles potential customer prospects, including registration, analysis, and follow-ups to be regular customers",
                            flag = 1,
                            icon = "fa-solid fa-handshake-simple",
                            modulectgid = 3,
                            modulename = "Prospect Customer Relationship",
                            modulesort = 3
                        },
                        new
                        {
                            moduleid = "104",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Manages relationships with potential agents, registration, evaluation, and communication to expand the operational network.",
                            flag = 1,
                            icon = "fa-solid fa-people-group",
                            modulectgid = 3,
                            modulename = "Prospect Agent Relationship",
                            modulesort = 4
                        },
                        new
                        {
                            moduleid = "105",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Records and manages the sales process, pricing and monitoring team performance and targets.",
                            flag = 1,
                            icon = "fa-solid fa-magnifying-glass-chart",
                            modulectgid = 3,
                            modulename = "Sales Management",
                            modulesort = 5
                        },
                        new
                        {
                            moduleid = "106",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Schedules and manages pickup from customers, including route optimization and field team coordination.",
                            flag = 1,
                            icon = "fa-solid fa-truck-moving",
                            modulectgid = 3,
                            modulename = "Pickup Management",
                            modulesort = 6
                        },
                        new
                        {
                            moduleid = "107",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Controls and manages the outbound shipment of goods from origin distribution centers to final destinations.",
                            flag = 1,
                            icon = "fa-solid fa-plane-departure",
                            modulectgid = 4,
                            modulename = "Outbound Control Library",
                            modulesort = 7
                        },
                        new
                        {
                            moduleid = "108",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Monitors and manages the movement of goods during intercity or inter-hub transportation.",
                            flag = 1,
                            icon = "fa-solid fa-truck-plane",
                            modulectgid = 4,
                            modulename = "Linehaul Control Library",
                            modulesort = 8
                        },
                        new
                        {
                            moduleid = "109",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Handles the receiving of incoming shipments from distribution centers until delivery to recipient.",
                            flag = 1,
                            icon = "fa-solid fa-plane-arrival",
                            modulectgid = 4,
                            modulename = "Inbound Control Library",
                            modulesort = 9
                        },
                        new
                        {
                            moduleid = "110",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Manages the return process due to incorrect shipments, damaged goods, or other return policies.",
                            flag = 1,
                            icon = "fa-solid fa-reply",
                            modulectgid = 4,
                            modulename = "Return Management",
                            modulesort = 10
                        },
                        new
                        {
                            moduleid = "111",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Inbound Processing Center",
                            flag = 0,
                            icon = "fa-solid fa-file-invoice",
                            modulectgid = 4,
                            modulename = "Billing Delivery System",
                            modulesort = 11
                        },
                        new
                        {
                            moduleid = "112",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Inbound Processing Center",
                            flag = 0,
                            icon = "fa-solid fa-credit-card",
                            modulectgid = 4,
                            modulename = "Credit Card System",
                            modulesort = 12
                        },
                        new
                        {
                            moduleid = "113",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Handles cash payments upon delivery, including transaction recording and coordination with couriers.",
                            flag = 1,
                            icon = "fa-solid fa-money-bill-wave",
                            modulectgid = 5,
                            modulename = "COD Collection",
                            modulesort = 13
                        },
                        new
                        {
                            moduleid = "114",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Provides real-time tracking for customers and internal teams to monitor shipment status and locations.",
                            flag = 1,
                            icon = "fa-solid fa-shoe-prints",
                            modulectgid = 4,
                            modulename = "Shipment Tracking Visibility",
                            modulesort = 14
                        },
                        new
                        {
                            moduleid = "115",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Addresses operational issues such as delivery delays, lost shipments, or customer complaints, and records provided solutions.",
                            flag = 1,
                            icon = "fa-solid fa-glasses",
                            modulectgid = 4,
                            modulename = "Problem Handling & Solution",
                            modulesort = 15
                        },
                        new
                        {
                            moduleid = "116",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Generates reports and data analysis related to operational performance, sales, and financial aspects to support strategic decision-making.",
                            flag = 1,
                            icon = "fa-solid fa-chart-line",
                            modulectgid = 5,
                            modulename = "Analytics & Reporting",
                            modulesort = 16
                        },
                        new
                        {
                            moduleid = "117",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Manages company financials, including transaction records, operational expenses, payment reconciliation, and invoicing.",
                            flag = 1,
                            icon = "fa-solid fa-sack-dollar",
                            modulectgid = 5,
                            modulename = "Finance Management",
                            modulesort = 17
                        },
                        new
                        {
                            moduleid = "118",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Provides customer support to answer inquiries, handle complaints, and offer shipment and service-related information.",
                            flag = 1,
                            icon = "fa-solid fa-headset",
                            modulectgid = 5,
                            modulename = "Customer Care Services",
                            modulesort = 18
                        },
                        new
                        {
                            moduleid = "119",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Ensures compliance with industry regulations and legal requirements, including document management and audit processes.",
                            flag = 1,
                            icon = "fa-solid fa-scale-balanced",
                            modulectgid = 6,
                            modulename = "Legal & Compliance",
                            modulesort = 19
                        },
                        new
                        {
                            moduleid = "120",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Manages connections with external systems, APIs, and third-party services to ensure seamless data exchange and interoperability.",
                            flag = 1,
                            icon = "fa-solid fa-code-compare",
                            modulectgid = 1,
                            modulename = "Integration Management",
                            modulesort = 20
                        },
                        new
                        {
                            moduleid = "121",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Ensures consistency across multiple systems by managing automated data imports, exports, and synchronization processes.",
                            flag = 1,
                            icon = "fa-solid fa-tags",
                            modulectgid = 2,
                            modulename = "NCS Pricing Module",
                            modulesort = 21
                        },
                        new
                        {
                            moduleid = "122",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            description = "Maintains data accuracy and reliability by implementing validation rules, detecting duplicates, and managing data correction processes.",
                            flag = 1,
                            icon = "fa-solid fa-list-check",
                            modulectgid = 2,
                            modulename = "Data Quality & Validation",
                            modulesort = 22
                        });
                });

            modelBuilder.Entity("iDss.X.Models.AppModuleCtg", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modulectgname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("modulectgsort")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.ToTable("app_modulectg");

                    b.HasData(
                        new
                        {
                            id = 1,
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            modulectgname = "Administration & System Management",
                            modulectgsort = 1
                        },
                        new
                        {
                            id = 2,
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            modulectgname = "Data Management",
                            modulectgsort = 2
                        },
                        new
                        {
                            id = 3,
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            modulectgname = "Pre-Delivery",
                            modulectgsort = 3
                        },
                        new
                        {
                            id = 4,
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            modulectgname = "In-Transit",
                            modulectgsort = 4
                        },
                        new
                        {
                            id = 5,
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            modulectgname = "Post-Delivery",
                            modulectgsort = 5
                        },
                        new
                        {
                            id = 6,
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            modulectgname = "Back Office",
                            modulectgsort = 6
                        },
                        new
                        {
                            id = 7,
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            modulectgname = "Warehouse Management",
                            modulectgsort = 7
                        });
                });

            modelBuilder.Entity("iDss.X.Models.Attachment", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("cpcode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<int>("isadjust")
                        .HasColumnType("int");

                    b.Property<string>("url1")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("url2")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("url3")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("url4")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("url5")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("urlsignature")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.ToTable("trx_attachment");
                });

            modelBuilder.Entity("iDss.X.Models.Bank", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("bankcode")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("bankname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("swiftcode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("id");

                    b.ToTable("mdt_bank");
                });

            modelBuilder.Entity("iDss.X.Models.Branch", b =>
                {
                    b.Property<int>("branchid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("branchid"));

                    b.Property<string>("addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr3")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("branchcode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("branchname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("branchsize")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("branchtype")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("citycode")
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("honame")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("latitude")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("longitude")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("phonealt")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("picname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("picno")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("postcode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("villid")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("branchid");

                    b.HasIndex("villid");

                    b.ToTable("mdt_branch");

                    b.HasData(
                        new
                        {
                            branchid = 1,
                            addr1 = "Jl Brigjen Katamso No.7, Kota Bambu Selatan, Palmerah, Jakarta Barat",
                            branchcode = "C1",
                            branchname = "Katamso Head Office",
                            branchtype = "Central",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            latitude = "-6.189083",
                            longitude = "106.800992",
                            villid = "3173071006"
                        },
                        new
                        {
                            branchid = 2,
                            addr1 = "Jl.Kemanggisan Raya No.19, Kota Bambu Selatan, Palmerah, Jakarta Barat",
                            branchcode = "C2",
                            branchname = "Kemanggisan Head Office",
                            branchtype = "Central",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            latitude = "-6.189993",
                            longitude = "106.792306",
                            villid = "3173071006"
                        });
                });

            modelBuilder.Entity("iDss.X.Models.BranchAccount", b =>
                {
                    b.Property<string>("acctno")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.HasIndex("acctno");

                    b.HasIndex("branchid");

                    b.ToTable("mdt_branchaccount");
                });

            modelBuilder.Entity("iDss.X.Models.CIF", b =>
                {
                    b.Property<string>("cif")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("cifname")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<int>("industryid")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("cif");

                    b.HasIndex("branchid");

                    b.HasIndex("industryid");

                    b.ToTable("mdt_cif");
                });

            modelBuilder.Entity("iDss.X.Models.CRO", b =>
                {
                    b.Property<string>("crocode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("croname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("joindate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("nip")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int?>("salestarget")
                        .HasColumnType("int");

                    b.HasKey("crocode");

                    b.HasIndex("branchid");

                    b.ToTable("mdt_cro");
                });

            modelBuilder.Entity("iDss.X.Models.CancelPickup", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("attachment")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("pickupno")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("pickupstatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("reasonid")
                        .HasColumnType("int");

                    b.Property<string>("reasonname")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("requestor")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("id");

                    b.HasIndex("pickupno");

                    b.ToTable("pum_cancelpickup");
                });

            modelBuilder.Entity("iDss.X.Models.Checkpoint", b =>
                {
                    b.Property<string>("cpcode")
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("cpname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.HasKey("cpcode");

                    b.ToTable("mdt_checkpoint");
                });

            modelBuilder.Entity("iDss.X.Models.CheckpointPool", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<int>("attempt")
                        .HasColumnType("int");

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("branchname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("couriercode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("cpcode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<DateTime>("cpdatetime")
                        .HasColumnType("datetime2");

                    b.Property<string>("cpname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<int>("islastpoint")
                        .HasColumnType("int");

                    b.Property<int>("isoversla")
                        .HasColumnType("int");

                    b.Property<string>("latitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("longitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int?>("reasonid")
                        .HasColumnType("int");

                    b.Property<string>("recipient")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int?>("relationid")
                        .HasColumnType("int");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.HasIndex("branchid");

                    b.HasIndex("couriercode");

                    b.HasIndex("reasonid");

                    b.HasIndex("relationid");

                    b.ToTable("trx_checkpointpool");
                });

            modelBuilder.Entity("iDss.X.Models.City", b =>
                {
                    b.Property<string>("cityid")
                        .HasMaxLength(4)
                        .HasColumnType("nvarchar(4)");

                    b.Property<string>("citycode")
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("citymerger")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("cityname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("hubcode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("provid")
                        .IsRequired()
                        .HasMaxLength(2)
                        .HasColumnType("nvarchar(2)");

                    b.HasKey("cityid");

                    b.HasIndex("provid");

                    b.ToTable("mdt_city");

                    b.HasData(
                        new
                        {
                            cityid = "1101",
                            citycode = "TTN",
                            citymerger = "ACEH SELATAN",
                            cityname = "KAB ACEH SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1102",
                            citycode = "KTN",
                            citymerger = "ACEH TENGGARA",
                            cityname = "KAB ACEH TENGGARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1103",
                            citycode = "LGS",
                            citymerger = "ACEH TIMUR",
                            cityname = "KAB ACEH TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1104",
                            citycode = "TKN",
                            citymerger = "ACEH TENGAH",
                            cityname = "KAB ACEH TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1105",
                            citycode = "MBO",
                            citymerger = "ACEH BARAT",
                            cityname = "KAB ACEH BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1106",
                            citycode = "JTH",
                            citymerger = "ACEH BESAR",
                            cityname = "KAB ACEH BESAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1107",
                            citycode = "SGI",
                            citymerger = "PIDIE",
                            cityname = "KAB PIDIE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1108",
                            citycode = "LSK",
                            citymerger = "ACEH UTARA",
                            cityname = "KAB ACEH UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1109",
                            citycode = "SNB",
                            citymerger = "SIMEULUE",
                            cityname = "KAB SIMEULUE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1110",
                            citycode = "SKL",
                            citymerger = "ACEH SINGKIL",
                            cityname = "KAB ACEH SINGKIL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1111",
                            citycode = "BIR",
                            citymerger = "BIREUEN",
                            cityname = "KAB BIREUEN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1112",
                            citycode = "BPD",
                            citymerger = "ACEH BARAT DAYA",
                            cityname = "KAB ACEH BARAT DAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1113",
                            citycode = "BKJ",
                            citymerger = "GAYO LUES",
                            cityname = "KAB GAYO LUES",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1114",
                            citycode = "CAG",
                            citymerger = "ACEH JAYA",
                            cityname = "KAB ACEH JAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1115",
                            citycode = "SKM",
                            citymerger = "NAGAN RAYA",
                            cityname = "KAB NAGAN RAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1116",
                            citycode = "KRB",
                            citymerger = "ACEH TAMIANG",
                            cityname = "KAB ACEH TAMIANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1117",
                            citycode = "STR",
                            citymerger = "BENER MERIAH",
                            cityname = "KAB BENER MERIAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1118",
                            citycode = "MRN",
                            citymerger = "PIDIE JAYA",
                            cityname = "KAB PIDIE JAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1171",
                            citycode = "BTJ",
                            citymerger = "BANDA ACEH",
                            cityname = "KOTA BANDA ACEH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1172",
                            citycode = "SBG",
                            citymerger = "SABANG",
                            cityname = "KOTA SABANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1173",
                            citycode = "LSW",
                            citymerger = "LHOKSEUMAWE",
                            cityname = "KOTA LHOKSEUMAWE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1174",
                            citycode = "LGS",
                            citymerger = "LANGSA",
                            cityname = "KOTA LANGSA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1175",
                            citycode = "SUS",
                            citymerger = "SUBULUSSALAM",
                            cityname = "KOTA SUBULUSSALAM",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTJ",
                            provid = "11"
                        },
                        new
                        {
                            cityid = "1201",
                            citycode = "SBG",
                            citymerger = "TAPANULI",
                            cityname = "KAB TAPANULI TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1202",
                            citycode = "TRT",
                            citymerger = "TAPANULI",
                            cityname = "KAB TAPANULI UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1203",
                            citycode = "PSP",
                            citymerger = "TAPANULI",
                            cityname = "KAB TAPANULI SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1204",
                            citycode = "GST",
                            citymerger = "NIAS",
                            cityname = "KAB NIAS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1205",
                            citycode = "STB",
                            citymerger = "LANGKAT",
                            cityname = "KAB LANGKAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1206",
                            citycode = "KBJ",
                            citymerger = "KARO",
                            cityname = "KAB KARO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1207",
                            citycode = "LBP",
                            citymerger = "DELI SERDANG",
                            cityname = "KAB DELI SERDANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1208",
                            citycode = "SIM",
                            citymerger = "SIMALUNGUN",
                            cityname = "KAB SIMALUNGUN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1209",
                            citycode = "KIS",
                            citymerger = "ASAHAN",
                            cityname = "KAB ASAHAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1210",
                            citycode = "RTP",
                            citymerger = "LABUHANBATU",
                            cityname = "KAB LABUHANBATU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1211",
                            citycode = "SKD",
                            citymerger = "DAIRI",
                            cityname = "KAB DAIRI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1212",
                            citycode = "BLG",
                            citymerger = "TOBA SAMOSIR",
                            cityname = "KAB TOBA SAMOSIR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1213",
                            citycode = "PYB",
                            citymerger = "MANDAILING NATAL",
                            cityname = "KAB MANDAILING NATAL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1214",
                            citycode = "TLD",
                            citymerger = "NIAS",
                            cityname = "KAB NIAS SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1215",
                            citycode = "SAL",
                            citymerger = "PAKPAK BHARAT",
                            cityname = "KAB PAKPAK BHARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1216",
                            citycode = "DLS",
                            citymerger = "HUMBANG HASUNDUTAN",
                            cityname = "KAB HUMBANG HASUNDUTAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1217",
                            citycode = "PRR",
                            citymerger = "SAMOSIR",
                            cityname = "KAB SAMOSIR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1218",
                            citycode = "SRH",
                            citymerger = "SERDANG BEDAGAI",
                            cityname = "KAB SERDANG BEDAGAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1219",
                            citycode = "LPM",
                            citymerger = "BATU BARA",
                            cityname = "KAB BATU BARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1220",
                            citycode = "GNT",
                            citymerger = "PADANG LAWAS",
                            cityname = "KAB PADANG LAWAS UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1221",
                            citycode = "SBH",
                            citymerger = "PADANG LAWAS",
                            cityname = "KAB PADANG LAWAS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1222",
                            citycode = "KPI",
                            citymerger = "LABUHANBATU",
                            cityname = "KAB LABUHANBATU SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1223",
                            citycode = "AKK",
                            citymerger = "LABUHANBATU",
                            cityname = "KAB LABUHANBATU UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1224",
                            citycode = "LTU",
                            citymerger = "NIAS",
                            cityname = "KAB NIAS UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1225",
                            citycode = "LHM",
                            citymerger = "NIAS",
                            cityname = "KAB NIAS BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1271",
                            citycode = "MES",
                            citymerger = "MEDAN",
                            cityname = "KOTA MEDAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1272",
                            citycode = "PMS",
                            citymerger = "PEMATANG SIANTAR",
                            cityname = "KOTA PEMATANG SIANTAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1273",
                            citycode = "SBG",
                            citymerger = "SIBOLGA",
                            cityname = "KOTA SIBOLGA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1274",
                            citycode = "TJB",
                            citymerger = "TANJUNG BALAI",
                            cityname = "KOTA TANJUNG BALAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1275",
                            citycode = "BNJ",
                            citymerger = "BINJAI",
                            cityname = "KOTA BINJAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1276",
                            citycode = "TBT",
                            citymerger = "TEBING TINGGI",
                            cityname = "KOTA TEBING TINGGI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1277",
                            citycode = "PSP",
                            citymerger = "PADANGSIDIMPUAN",
                            cityname = "KOTA PADANGSIDIMPUAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1278",
                            citycode = "GNS",
                            citymerger = "GUNUNGSITOLI",
                            cityname = "KOTA GUNUNGSITOLI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MES",
                            provid = "12"
                        },
                        new
                        {
                            cityid = "1301",
                            citycode = "PSR",
                            citymerger = "PESISIR SELATAN",
                            cityname = "KAB PESISIR SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1302",
                            citycode = "SOL",
                            citymerger = "SOLOK",
                            cityname = "KAB SOLOK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1303",
                            citycode = "SJJ",
                            citymerger = "SIJUNJUNG",
                            cityname = "KAB SIJUNJUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1304",
                            citycode = "TDR",
                            citymerger = "TANAH DATAR",
                            cityname = "KAB TANAH DATAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1305",
                            citycode = "PPR",
                            citymerger = "PADANG PARIAMAN",
                            cityname = "KAB PADANG PARIAMAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1306",
                            citycode = "AGM",
                            citymerger = "AGAM",
                            cityname = "KAB AGAM",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1307",
                            citycode = "LPH",
                            citymerger = "LIMA PULUH KOTA",
                            cityname = "KAB LIMA PULUH KOTA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1308",
                            citycode = "PSM",
                            citymerger = "PASAMAN",
                            cityname = "KAB PASAMAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1309",
                            citycode = "MTW",
                            citymerger = "KEPULAUAN MENTAWAI",
                            cityname = "KAB KEPULAUAN MENTAWAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1310",
                            citycode = "DMR",
                            citymerger = "DHARMASRAYA",
                            cityname = "KAB DHARMASRAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1311",
                            citycode = "SSL",
                            citymerger = "SOLOK",
                            cityname = "KAB SOLOK SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1312",
                            citycode = "PSB",
                            citymerger = "PASAMAN",
                            cityname = "KAB PASAMAN BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1371",
                            citycode = "PDG",
                            citymerger = "PADANG",
                            cityname = "KOTA PADANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1372",
                            citycode = "SLK",
                            citymerger = "SOLOK",
                            cityname = "KOTA SOLOK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1373",
                            citycode = "SWL",
                            citymerger = "SAWAHLUNTO",
                            cityname = "KOTA SAWAHLUNTO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1374",
                            citycode = "PPJ",
                            citymerger = "PADANG PANJANG",
                            cityname = "KOTA PADANG PANJANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1375",
                            citycode = "BKT",
                            citymerger = "BUKITTINGGI",
                            cityname = "KOTA BUKITTINGGI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1376",
                            citycode = "PKY",
                            citymerger = "PAYAKUMBUH",
                            cityname = "KOTA PAYAKUMBUH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1377",
                            citycode = "PRM",
                            citymerger = "PARIAMAN",
                            cityname = "KOTA PARIAMAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PDG",
                            provid = "13"
                        },
                        new
                        {
                            cityid = "1401",
                            citycode = "BKN",
                            citymerger = "KAMPAR",
                            cityname = "KAB KAMPAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1402",
                            citycode = "RGT",
                            citymerger = "INDRAGIRI HULU",
                            cityname = "KAB INDRAGIRI HULU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1403",
                            citycode = "BLS",
                            citymerger = "BENGKALIS",
                            cityname = "KAB BENGKALIS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1404",
                            citycode = "TBH",
                            citymerger = "INDRAGIRI HILIR",
                            cityname = "KAB INDRAGIRI HILIR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1405",
                            citycode = "PKK",
                            citymerger = "PELALAWAN",
                            cityname = "KAB PELALAWAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1406",
                            citycode = "PRP",
                            citymerger = "ROKAN HULU",
                            cityname = "KAB ROKAN HULU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1407",
                            citycode = "UJT",
                            citymerger = "ROKAN HILIR",
                            cityname = "KAB ROKAN HILIR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1408",
                            citycode = "SAK",
                            citymerger = "SIAK",
                            cityname = "KAB SIAK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1409",
                            citycode = "TLK",
                            citymerger = "KUANTAN SINGINGI",
                            cityname = "KAB KUANTAN SINGINGI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1410",
                            citycode = "TTG",
                            citymerger = "KEPULAUAN MERANTI",
                            cityname = "KAB KEPULAUAN MERANTI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1471",
                            citycode = "PKU",
                            citymerger = "PEKANBARU",
                            cityname = "KOTA PEKANBARU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1472",
                            citycode = "DUM",
                            citymerger = "DUMAI",
                            cityname = "KOTA DUMAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKU",
                            provid = "14"
                        },
                        new
                        {
                            cityid = "1501",
                            citycode = "KRC",
                            citymerger = "KERINCI",
                            cityname = "KAB KERINCI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1502",
                            citycode = "MRG",
                            citymerger = "MERANGIN",
                            cityname = "KAB MERANGIN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1503",
                            citycode = "SAR",
                            citymerger = "SAROLANGUN",
                            cityname = "KAB SAROLANGUN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1504",
                            citycode = "MBO",
                            citymerger = "BATANGHARI",
                            cityname = "KAB BATANGHARI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1505",
                            citycode = "MJA",
                            citymerger = "MUARO JAMBI",
                            cityname = "KAB MUARO JAMBI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1506",
                            citycode = "KTU",
                            citymerger = "TANJUNG JABUNG",
                            cityname = "KAB TANJUNG JABUNG BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1507",
                            citycode = "MSA",
                            citymerger = "TANJUNG JABUNG",
                            cityname = "KAB TANJUNG JABUNG TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1508",
                            citycode = "MRB",
                            citymerger = "BUNGO",
                            cityname = "KAB BUNGO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MRB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1509",
                            citycode = "MTE",
                            citymerger = "TEBO",
                            cityname = "KAB TEBO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1571",
                            citycode = "DJB",
                            citymerger = "JAMBI",
                            cityname = "KOTA JAMBI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1572",
                            citycode = "SPE",
                            citymerger = "SUNGAI PENUH",
                            cityname = "KOTA SUNGAI PENUH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJB",
                            provid = "15"
                        },
                        new
                        {
                            cityid = "1601",
                            citycode = "BTA",
                            citymerger = "OGAN KOMERING ULU",
                            cityname = "KAB OGAN KOMERING ULU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1602",
                            citycode = "OKI",
                            citymerger = "OGAN KOMERING ILIR",
                            cityname = "KAB OGAN KOMERING ILIR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1603",
                            citycode = "MRE",
                            citymerger = "MUARA ENIM",
                            cityname = "KAB MUARA ENIM",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1604",
                            citycode = "LHT",
                            citymerger = "LAHAT",
                            cityname = "KAB LAHAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1605",
                            citycode = "MRA",
                            citymerger = "MUSI RAWAS",
                            cityname = "KAB MUSI RAWAS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1606",
                            citycode = "MBA",
                            citymerger = "MUSI BANYUASIN",
                            cityname = "KAB MUSI BANYUASIN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1607",
                            citycode = "BYA",
                            citymerger = "BANYUASIN",
                            cityname = "KAB BANYUASIN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1608",
                            citycode = "OKU",
                            citymerger = "OGAN KOMERING ULU TIMUR",
                            cityname = "KAB OGAN KOMERING ULU TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1609",
                            citycode = "OKU",
                            citymerger = "OGAN KOMERING ULU SELATAN",
                            cityname = "KAB OGAN KOMERING ULU SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1610",
                            citycode = "OGI",
                            citymerger = "OGAN ILIR",
                            cityname = "KAB OGAN ILIR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1611",
                            citycode = "PDO",
                            citymerger = "EMPAT LAWANG",
                            cityname = "KAB EMPAT LAWANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1612",
                            citycode = "PNA",
                            citymerger = "PENUKAL ABAB",
                            cityname = "KAB PENUKAL ABAB",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1613",
                            citycode = "MRU",
                            citymerger = "MUSI RAWAS UTARA",
                            cityname = "KAB MUSI RAWAS UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1671",
                            citycode = "PLM",
                            citymerger = "PALEMBANG",
                            cityname = "KOTA PALEMBANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1672",
                            citycode = "PGA",
                            citymerger = "PAGAR ALAM",
                            cityname = "KOTA PAGAR ALAM",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1673",
                            citycode = "LLG",
                            citymerger = "LUBUK LINGGAU",
                            cityname = "KOTA LUBUK LINGGAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1674",
                            citycode = "PRB",
                            citymerger = "PRABUMULIH",
                            cityname = "KOTA PRABUMULIH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLM",
                            provid = "16"
                        },
                        new
                        {
                            cityid = "1701",
                            citycode = "MNN",
                            citymerger = "BENGKULU SELATAN",
                            cityname = "KAB BENGKULU SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1702",
                            citycode = "CRP",
                            citymerger = "REJANG LEBONG",
                            cityname = "KAB REJANG LEBONG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1703",
                            citycode = "ARM",
                            citymerger = "BENGKULU UTARA",
                            cityname = "KAB BENGKULU UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1704",
                            citycode = "BTN",
                            citymerger = "KAUR",
                            cityname = "KAB KAUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1705",
                            citycode = "SLM",
                            citymerger = "SELUMA",
                            cityname = "KAB SELUMA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1706",
                            citycode = "MPC",
                            citymerger = "MUKO MUKO",
                            cityname = "KAB MUKO MUKO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1707",
                            citycode = "LBG",
                            citymerger = "LEBONG",
                            cityname = "KAB LEBONG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1708",
                            citycode = "KPH",
                            citymerger = "KEPAHIANG",
                            cityname = "KAB KEPAHIANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1709",
                            citycode = "KRT",
                            citymerger = "BENGKULU TENGAH",
                            cityname = "KAB BENGKULU TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1771",
                            citycode = "BKS",
                            citymerger = "BENGKULU",
                            cityname = "KOTA BENGKULU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKS",
                            provid = "17"
                        },
                        new
                        {
                            cityid = "1801",
                            citycode = "LPS",
                            citymerger = "LAMPUNG SELATAN",
                            cityname = "KAB LAMPUNG SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1802",
                            citycode = "LPT",
                            citymerger = "LAMPUNG TENGAH",
                            cityname = "KAB LAMPUNG TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1803",
                            citycode = "LPU",
                            citymerger = "LAMPUNG UTARA",
                            cityname = "KAB LAMPUNG UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1804",
                            citycode = "LPB",
                            citymerger = "LAMPUNG BARAT",
                            cityname = "KAB LAMPUNG BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1805",
                            citycode = "TBA",
                            citymerger = "TULANG BAWANG",
                            cityname = "KAB TULANG BAWANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1806",
                            citycode = "TGS",
                            citymerger = "TANGGAMUS",
                            cityname = "KAB TANGGAMUS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1807",
                            citycode = "LTM",
                            citymerger = "LAMPUNG TIMUR",
                            cityname = "KAB LAMPUNG TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1808",
                            citycode = "WKN",
                            citymerger = "WAY KANAN",
                            cityname = "KAB WAY KANAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1809",
                            citycode = "PSR",
                            citymerger = "PESAWARAN",
                            cityname = "KAB PESAWARAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1810",
                            citycode = "PSW",
                            citymerger = "PRINGSEWU",
                            cityname = "KAB PRINGSEWU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1811",
                            citycode = "TBM",
                            citymerger = "MESUJI",
                            cityname = "KAB MESUJI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1812",
                            citycode = "TBB",
                            citymerger = "TULANG BAWANG",
                            cityname = "KAB TULANG BAWANG BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1813",
                            citycode = "LPB",
                            citymerger = "PESISIR BARAT",
                            cityname = "KAB PESISIR BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1871",
                            citycode = "TKG",
                            citymerger = "BANDAR LAMPUNG",
                            cityname = "KOTA BANDAR LAMPUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1872",
                            citycode = "TKG",
                            citymerger = "METRO",
                            cityname = "KOTA METRO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TKG",
                            provid = "18"
                        },
                        new
                        {
                            cityid = "1901",
                            citycode = "SGL",
                            citymerger = "BANGKA",
                            cityname = "KAB BANGKA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PGK",
                            provid = "19"
                        },
                        new
                        {
                            cityid = "1902",
                            citycode = "TJQ",
                            citymerger = "BELITUNG",
                            cityname = "KAB BELITUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TJQ",
                            provid = "19"
                        },
                        new
                        {
                            cityid = "1903",
                            citycode = "TBL",
                            citymerger = "BANGKA SELATAN",
                            cityname = "KAB BANGKA SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PGK",
                            provid = "19"
                        },
                        new
                        {
                            cityid = "1904",
                            citycode = "KBA",
                            citymerger = "BANGKA TENGAH",
                            cityname = "KAB BANGKA TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PGK",
                            provid = "19"
                        },
                        new
                        {
                            cityid = "1905",
                            citycode = "MTK",
                            citymerger = "BANGKA BARAT",
                            cityname = "KAB BANGKA BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PGK",
                            provid = "19"
                        },
                        new
                        {
                            cityid = "1906",
                            citycode = "MGR",
                            citymerger = "BELITUNG TIMUR",
                            cityname = "KAB BELITUNG TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TJQ",
                            provid = "19"
                        },
                        new
                        {
                            cityid = "1971",
                            citycode = "PGK",
                            citymerger = "PANGKAL PINANG",
                            cityname = "KOTA PANGKAL PINANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PGK",
                            provid = "19"
                        },
                        new
                        {
                            cityid = "2101",
                            citycode = "TNJ",
                            citymerger = "BINTAN",
                            cityname = "KAB BINTAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TNJ",
                            provid = "21"
                        },
                        new
                        {
                            cityid = "2102",
                            citycode = "TBK",
                            citymerger = "KARIMUN",
                            cityname = "KAB KARIMUN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTH",
                            provid = "21"
                        },
                        new
                        {
                            cityid = "2103",
                            citycode = "NTX",
                            citymerger = "NATUNA",
                            cityname = "KAB NATUNA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "NTX",
                            provid = "21"
                        },
                        new
                        {
                            cityid = "2104",
                            citycode = "DKL",
                            citymerger = "LINGGA",
                            cityname = "KAB LINGGA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TNJ",
                            provid = "21"
                        },
                        new
                        {
                            cityid = "2105",
                            citycode = "NWK",
                            citymerger = "KEPULAUAN ANAMBAS",
                            cityname = "KAB KEPULAUAN ANAMBAS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTH",
                            provid = "21"
                        },
                        new
                        {
                            cityid = "2171",
                            citycode = "BTH",
                            citymerger = "BATAM",
                            cityname = "KOTA BATAM",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BTH",
                            provid = "21"
                        },
                        new
                        {
                            cityid = "2172",
                            citycode = "TNJ",
                            citymerger = "TANJUNG PINANG",
                            cityname = "KOTA TANJUNG PINANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TNJ",
                            provid = "21"
                        },
                        new
                        {
                            cityid = "3101",
                            citycode = "PPJ",
                            citymerger = "KEPULAUAN SERIBU",
                            cityname = "KAB ADM KEP SERIBU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CGK",
                            provid = "31"
                        },
                        new
                        {
                            cityid = "3171",
                            citycode = "CGK",
                            citymerger = "JAKARTA PUSAT",
                            cityname = "KOTA ADM JAKARTA PUSAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CGK",
                            provid = "31"
                        },
                        new
                        {
                            cityid = "3172",
                            citycode = "CGK",
                            citymerger = "JAKARTA UTARA",
                            cityname = "KOTA ADM JAKARTA UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CGK",
                            provid = "31"
                        },
                        new
                        {
                            cityid = "3173",
                            citycode = "CGK",
                            citymerger = "JAKARTA BARAT",
                            cityname = "KOTA ADM JAKARTA BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CGK",
                            provid = "31"
                        },
                        new
                        {
                            cityid = "3174",
                            citycode = "CGK",
                            citymerger = "JAKARTA SELATAN",
                            cityname = "KOTA ADM JAKARTA SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CGK",
                            provid = "31"
                        },
                        new
                        {
                            cityid = "3175",
                            citycode = "CGK",
                            citymerger = "JAKARTA TIMUR",
                            cityname = "KOTA ADM JAKARTA TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CGK",
                            provid = "31"
                        },
                        new
                        {
                            cityid = "3201",
                            citycode = "BOO",
                            citymerger = "BOGOR",
                            cityname = "KAB BOGOR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BOO",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3202",
                            citycode = "SMI",
                            citymerger = "SUKABUMI",
                            cityname = "KAB SUKABUMI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SMI",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3203",
                            citycode = "CIJ",
                            citymerger = "CIANJUR",
                            cityname = "KAB CIANJUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CIJ",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3204",
                            citycode = "BDO",
                            citymerger = "BANDUNG",
                            cityname = "KAB BANDUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDO",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3205",
                            citycode = "GRT",
                            citymerger = "GARUT",
                            cityname = "KAB GARUT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "GRT",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3206",
                            citycode = "TSY",
                            citymerger = "TASIKMALAYA",
                            cityname = "KAB TASIKMALAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TSY",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3207",
                            citycode = "CMI",
                            citymerger = "CIAMIS",
                            cityname = "KAB CIAMIS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TSY",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3208",
                            citycode = "KGN",
                            citymerger = "KUNINGAN",
                            cityname = "KAB KUNINGAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KGN",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3209",
                            citycode = "CBN",
                            citymerger = "CIREBON",
                            cityname = "KAB CIREBON",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CBN",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3210",
                            citycode = "KJT",
                            citymerger = "MAJALENGKA",
                            cityname = "KAB MAJALENGKA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KJT",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3211",
                            citycode = "BDO",
                            citymerger = "SUMEDANG",
                            cityname = "KAB SUMEDANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDO",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3212",
                            citycode = "CBN",
                            citymerger = "INDRAMAYU",
                            cityname = "KAB INDRAMAYU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CBN",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3213",
                            citycode = "CKP",
                            citymerger = "SUBANG",
                            cityname = "KAB SUBANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CKP",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3214",
                            citycode = "CKP",
                            citymerger = "PURWAKARTA",
                            cityname = "KAB PURWAKARTA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CKP",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3215",
                            citycode = "CKP",
                            citymerger = "KARAWANG",
                            cityname = "KAB KARAWANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CKP",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3216",
                            citycode = "BKI",
                            citymerger = "BEKASI",
                            cityname = "KAB BEKASI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKI",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3217",
                            citycode = "BDO",
                            citymerger = "BANDUNG",
                            cityname = "KAB BANDUNG BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDO",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3218",
                            citycode = "BJR",
                            citymerger = "PANGANDARAN",
                            cityname = "KAB PANGANDARAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TSY",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3271",
                            citycode = "BOO",
                            citymerger = "BOGOR",
                            cityname = "KOTA BOGOR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BOO",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3272",
                            citycode = "SMI",
                            citymerger = "SUKABUMI",
                            cityname = "KOTA SUKABUMI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SMI",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3273",
                            citycode = "BDO",
                            citymerger = "BANDUNG",
                            cityname = "KOTA BANDUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDO",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3274",
                            citycode = "CBN",
                            citymerger = "CIREBON",
                            cityname = "KOTA CIREBON",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CBN",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3275",
                            citycode = "BKI",
                            citymerger = "BEKASI",
                            cityname = "KOTA BEKASI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BKI",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3276",
                            citycode = "DPK",
                            citymerger = "DEPOK",
                            cityname = "KOTA DEPOK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPK",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3277",
                            citycode = "BDO",
                            citymerger = "CIMAHI",
                            cityname = "KOTA CIMAHI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDO",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3278",
                            citycode = "TSY",
                            citymerger = "TASIKMALAYA",
                            cityname = "KOTA TASIKMALAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TSY",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3279",
                            citycode = "BJR",
                            citymerger = "BANJAR",
                            cityname = "KOTA BANJAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TSY",
                            provid = "32"
                        },
                        new
                        {
                            cityid = "3301",
                            citycode = "CXP",
                            citymerger = "CILACAP",
                            cityname = "KAB CILACAP",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CXP",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3302",
                            citycode = "PWO",
                            citymerger = "BANYUMAS",
                            cityname = "KAB BANYUMAS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PWO",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3303",
                            citycode = "PWL",
                            citymerger = "PURBALINGGA",
                            cityname = "KAB PURBALINGGA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PWO",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3304",
                            citycode = "BJG",
                            citymerger = "BANJARNEGARA",
                            cityname = "KAB BANJARNEGARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BJG",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3305",
                            citycode = "KBM",
                            citymerger = "KEBUMEN",
                            cityname = "KAB KEBUMEN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KBM",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3306",
                            citycode = "MGL",
                            citymerger = "PURWOREJO",
                            cityname = "KAB PURWOREJO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MGL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3307",
                            citycode = "BJG",
                            citymerger = "WONOSOBO",
                            cityname = "KAB WONOSOBO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BJG",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3308",
                            citycode = "MGL",
                            citymerger = "MAGELANG",
                            cityname = "KAB MAGELANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MGL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3309",
                            citycode = "BYL",
                            citymerger = "BOYOLALI",
                            cityname = "KAB BOYOLALI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOC",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3310",
                            citycode = "KLN",
                            citymerger = "KLATEN",
                            cityname = "KAB KLATEN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KLN",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3311",
                            citycode = "SKH",
                            citymerger = "SUKOHARJO",
                            cityname = "KAB SUKOHARJO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOC",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3312",
                            citycode = "SKH",
                            citymerger = "WONOGIRI",
                            cityname = "KAB WONOGIRI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SKH",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3313",
                            citycode = "SOC",
                            citymerger = "KARANGANYAR",
                            cityname = "KAB KARANGANYAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOC",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3314",
                            citycode = "SRN",
                            citymerger = "SRAGEN",
                            cityname = "KAB SRAGEN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOC",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3315",
                            citycode = "PWD",
                            citymerger = "GROBOGAN",
                            cityname = "KAB GROBOGAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PWD",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3316",
                            citycode = "BLA",
                            citymerger = "BLORA",
                            cityname = "KAB BLORA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PWD",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3317",
                            citycode = "RBG",
                            citymerger = "REMBANG",
                            cityname = "KAB REMBANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PTI",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3318",
                            citycode = "PTI",
                            citymerger = "PATI",
                            cityname = "KAB PATI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PTI",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3319",
                            citycode = "KDS",
                            citymerger = "KUDUS",
                            cityname = "KAB KUDUS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDS",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3320",
                            citycode = "KDS",
                            citymerger = "JEPARA",
                            cityname = "KAB JEPARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDS",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3321",
                            citycode = "DMK",
                            citymerger = "DEMAK",
                            cityname = "KAB DEMAK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DMK",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3322",
                            citycode = "SRG",
                            citymerger = "SEMARANG",
                            cityname = "KAB SEMARANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRG",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3323",
                            citycode = "MGL",
                            citymerger = "TEMANGGUNG",
                            cityname = "KAB TEMANGGUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MGL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3324",
                            citycode = "SRG",
                            citymerger = "KENDAL",
                            cityname = "KAB KENDAL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRG",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3325",
                            citycode = "PKL",
                            citymerger = "BATANG",
                            cityname = "KAB BATANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3326",
                            citycode = "PKL",
                            citymerger = "PEKALONGAN",
                            cityname = "KAB PEKALONGAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3327",
                            citycode = "TGL",
                            citymerger = "PEMALANG",
                            cityname = "KAB PEMALANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TGL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3328",
                            citycode = "TGL",
                            citymerger = "TEGAL",
                            cityname = "KAB TEGAL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TGL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3329",
                            citycode = "TGL",
                            citymerger = "BREBES",
                            cityname = "KAB BREBES",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TGL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3371",
                            citycode = "MGL",
                            citymerger = "MAGELANG",
                            cityname = "KOTA MAGELANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MGL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3372",
                            citycode = "SOC",
                            citymerger = "SURAKARTA",
                            cityname = "KOTA SURAKARTA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOC",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3373",
                            citycode = "SLT",
                            citymerger = "SALATIGA",
                            cityname = "KOTA SALATIGA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SLT",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3374",
                            citycode = "SRG",
                            citymerger = "SEMARANG",
                            cityname = "KOTA SEMARANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRG",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3375",
                            citycode = "PKL",
                            citymerger = "PEKALONGAN",
                            cityname = "KOTA PEKALONGAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3376",
                            citycode = "TGL",
                            citymerger = "TEGAL",
                            cityname = "KOTA TEGAL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TGL",
                            provid = "33"
                        },
                        new
                        {
                            cityid = "3401",
                            citycode = "YOG",
                            citymerger = "KULON PROGO",
                            cityname = "KAB KULON PROGO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "YOG",
                            provid = "34"
                        },
                        new
                        {
                            cityid = "3402",
                            citycode = "YOG",
                            citymerger = "BANTUL",
                            cityname = "KAB BANTUL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "YOG",
                            provid = "34"
                        },
                        new
                        {
                            cityid = "3403",
                            citycode = "YOG",
                            citymerger = "GUNUNGKIDUL",
                            cityname = "KAB GUNUNGKIDUL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "YOG",
                            provid = "34"
                        },
                        new
                        {
                            cityid = "3404",
                            citycode = "YOG",
                            citymerger = "SLEMAN",
                            cityname = "KAB SLEMAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "YOG",
                            provid = "34"
                        },
                        new
                        {
                            cityid = "3471",
                            citycode = "YOG",
                            citymerger = "YOGYAKARTA",
                            cityname = "KOTA YOGYAKARTA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "YOG",
                            provid = "34"
                        },
                        new
                        {
                            cityid = "3501",
                            citycode = "PCT",
                            citymerger = "PACITAN",
                            cityname = "KAB PACITAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3502",
                            citycode = "MDN",
                            citymerger = "PONOROGO",
                            cityname = "KAB PONOROGO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3503",
                            citycode = "KDR",
                            citymerger = "TRENGGALEK",
                            cityname = "KAB TRENGGALEK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDR",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3504",
                            citycode = "KDR",
                            citymerger = "TULUNGAGUNG",
                            cityname = "KAB TULUNGAGUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDR",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3505",
                            citycode = "KDR",
                            citymerger = "BLITAR",
                            cityname = "KAB BLITAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDR",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3506",
                            citycode = "KDR",
                            citymerger = "KEDIRI",
                            cityname = "KAB KEDIRI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDR",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3507",
                            citycode = "MLG",
                            citymerger = "MALANG",
                            cityname = "KAB MALANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MLG",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3508",
                            citycode = "JBB",
                            citymerger = "LUMAJANG",
                            cityname = "KAB LUMAJANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "JBB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3509",
                            citycode = "JBB",
                            citymerger = "JEMBER",
                            cityname = "KAB JEMBER",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "JBB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3510",
                            citycode = "BYW",
                            citymerger = "BANYUWANGI",
                            cityname = "KAB BANYUWANGI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BYW",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3511",
                            citycode = "SBD",
                            citymerger = "BONDOWOSO",
                            cityname = "KAB BONDOWOSO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SBD",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3512",
                            citycode = "SBD",
                            citymerger = "SITUBONDO",
                            cityname = "KAB SITUBONDO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SBD",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3513",
                            citycode = "PBL",
                            citymerger = "PROBOLINGGO",
                            cityname = "KAB PROBOLINGGO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PBL",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3514",
                            citycode = "PSN",
                            citymerger = "PASURUAN",
                            cityname = "KAB PASURUAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PSN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3515",
                            citycode = "SDA",
                            citymerger = "SIDOARJO",
                            cityname = "KAB SIDOARJO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SDA",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3516",
                            citycode = "SDA",
                            citymerger = "MOJOKERTO",
                            cityname = "KAB MOJOKERTO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SDA",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3517",
                            citycode = "KDR",
                            citymerger = "JOMBANG",
                            cityname = "KAB JOMBANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDR",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3518",
                            citycode = "KDR",
                            citymerger = "NGANJUK",
                            cityname = "KAB NGANJUK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDR",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3519",
                            citycode = "MDN",
                            citymerger = "MADIUN",
                            cityname = "KAB MADIUN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3520",
                            citycode = "MGT",
                            citymerger = "MAGETAN",
                            cityname = "KAB MAGETAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3521",
                            citycode = "NGW",
                            citymerger = "NGAWI",
                            cityname = "KAB NGAWI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3522",
                            citycode = "BJN",
                            citymerger = "BOJONEGORO",
                            cityname = "KAB BOJONEGORO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BJN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3523",
                            citycode = "SUB",
                            citymerger = "TUBAN",
                            cityname = "KAB TUBAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SUB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3524",
                            citycode = "LMG",
                            citymerger = "LAMONGAN",
                            cityname = "KAB LAMONGAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "LMG",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3525",
                            citycode = "SUB",
                            citymerger = "GRESIK",
                            cityname = "KAB GRESIK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SUB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3526",
                            citycode = "SUP",
                            citymerger = "BANGKALAN",
                            cityname = "KAB BANGKALAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SUB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3527",
                            citycode = "SPG",
                            citymerger = "SAMPANG",
                            cityname = "KAB SAMPANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SPG",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3528",
                            citycode = "SUP",
                            citymerger = "PAMEKASAN",
                            cityname = "KAB PAMEKASAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SUB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3529",
                            citycode = "SUP",
                            citymerger = "SUMENEP",
                            cityname = "KAB SUMENEP",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SUB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3571",
                            citycode = "KDR",
                            citymerger = "KEDIRI",
                            cityname = "KOTA KEDIRI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDR",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3572",
                            citycode = "KDR",
                            citymerger = "BLITAR",
                            cityname = "KOTA BLITAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDR",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3573",
                            citycode = "MLG",
                            citymerger = "MALANG",
                            cityname = "KOTA MALANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MLG",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3574",
                            citycode = "PBL",
                            citymerger = "PROBOLINGGO",
                            cityname = "KOTA PROBOLINGGO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SUB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3575",
                            citycode = "PSN",
                            citymerger = "PASURUAN",
                            cityname = "KOTA PASURUAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PSN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3576",
                            citycode = "SDA",
                            citymerger = "MOJOKERTO",
                            cityname = "KOTA MOJOKERTO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SDA",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3577",
                            citycode = "MDN",
                            citymerger = "MADIUN",
                            cityname = "KOTA MADIUN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDN",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3578",
                            citycode = "SUB",
                            citymerger = "SURABAYA",
                            cityname = "KOTA SURABAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SUB",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3579",
                            citycode = "MLG",
                            citymerger = "BATU",
                            cityname = "KOTA BATU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MLG",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3591",
                            citycode = "BXW",
                            citymerger = "BAWEAN",
                            cityname = "PULAU BAWEAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BXW",
                            provid = "35"
                        },
                        new
                        {
                            cityid = "3601",
                            citycode = "CLG",
                            citymerger = "PANDEGLANG",
                            cityname = "KAB PANDEGLANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CLG",
                            provid = "36"
                        },
                        new
                        {
                            cityid = "3602",
                            citycode = "CLG",
                            citymerger = "LEBAK",
                            cityname = "KAB LEBAK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CLG",
                            provid = "36"
                        },
                        new
                        {
                            cityid = "3603",
                            citycode = "TNG",
                            citymerger = "TANGERANG",
                            cityname = "KAB TANGERANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TNG",
                            provid = "36"
                        },
                        new
                        {
                            cityid = "3604",
                            citycode = "CLG",
                            citymerger = "SERANG",
                            cityname = "KAB SERANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CLG",
                            provid = "36"
                        },
                        new
                        {
                            cityid = "3671",
                            citycode = "TNG",
                            citymerger = "TANGERANG",
                            cityname = "KOTA TANGERANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TNG",
                            provid = "36"
                        },
                        new
                        {
                            cityid = "3672",
                            citycode = "CLG",
                            citymerger = "CILEGON",
                            cityname = "KOTA CILEGON",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CLG",
                            provid = "36"
                        },
                        new
                        {
                            cityid = "3673",
                            citycode = "CLG",
                            citymerger = "SERANG",
                            cityname = "KOTA SERANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "CLG",
                            provid = "36"
                        },
                        new
                        {
                            cityid = "3674",
                            citycode = "TNG",
                            citymerger = "TANGERANG",
                            cityname = "KOTA TANGERANG SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TNG",
                            provid = "36"
                        },
                        new
                        {
                            cityid = "5101",
                            citycode = "DPS",
                            citymerger = "JEMBRANA",
                            cityname = "KAB JEMBRANA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPS",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5102",
                            citycode = "DPS",
                            citymerger = "TABANAN",
                            cityname = "KAB TABANAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPS",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5103",
                            citycode = "DPS",
                            citymerger = "BADUNG",
                            cityname = "KAB BADUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPS",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5104",
                            citycode = "GIR",
                            citymerger = "GIANYAR",
                            cityname = "KAB GIANYAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "GIR",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5105",
                            citycode = "DPS",
                            citymerger = "KLUNGKUNG",
                            cityname = "KAB KLUNGKUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPS",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5106",
                            citycode = "DPS",
                            citymerger = "BANGLI",
                            cityname = "KAB BANGLI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPS",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5107",
                            citycode = "DPS",
                            citymerger = "KARANGASEM",
                            cityname = "KAB KARANGASEM",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPS",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5108",
                            citycode = "DPS",
                            citymerger = "BULELENG",
                            cityname = "KAB BULELENG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPS",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5171",
                            citycode = "DPS",
                            citymerger = "DENPASAR",
                            cityname = "KOTA DENPASAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DPS",
                            provid = "51"
                        },
                        new
                        {
                            cityid = "5201",
                            citycode = "AMI",
                            citymerger = "LOMBOK BARAT",
                            cityname = "KAB LOMBOK BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5202",
                            citycode = "PYA",
                            citymerger = "LOMBOK TENGAH",
                            cityname = "KAB LOMBOK TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5203",
                            citycode = "PYA",
                            citymerger = "LOMBOK TIMUR",
                            cityname = "KAB LOMBOK TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5204",
                            citycode = "LYK",
                            citymerger = "SUMBAWA",
                            cityname = "KAB SUMBAWA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5205",
                            citycode = "DPU",
                            citymerger = "DOMPU",
                            cityname = "KAB DOMPU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5206",
                            citycode = "BMU",
                            citymerger = "BIMA",
                            cityname = "KAB BIMA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5207",
                            citycode = "SWQ",
                            citymerger = "SUMBAWA BARAT",
                            cityname = "KAB SUMBAWA BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5208",
                            citycode = "PYA",
                            citymerger = "LOMBOK UTARA",
                            cityname = "KAB LOMBOK UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5271",
                            citycode = "AMI",
                            citymerger = "MATARAM",
                            cityname = "KOTA MATARAM",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5272",
                            citycode = "BMU",
                            citymerger = "BIMA",
                            cityname = "KOTA BIMA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMI",
                            provid = "52"
                        },
                        new
                        {
                            cityid = "5301",
                            citycode = "KOE",
                            citymerger = "KUPANG",
                            cityname = "KAB KUPANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5302",
                            citycode = "SOE",
                            citymerger = "TIMOR TENGAH SELATAN",
                            cityname = "KAB TIMOR TENGAH SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5303",
                            citycode = "KEF",
                            citymerger = "TIMOR TENGAH UTARA",
                            cityname = "KAB TIMOR TENGAH UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5304",
                            citycode = "ATB",
                            citymerger = "BELU",
                            cityname = "KAB BELU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5305",
                            citycode = "ARD",
                            citymerger = "ALOR",
                            cityname = "KAB ALOR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5306",
                            citycode = "LWE",
                            citymerger = "FLORES TIMUR",
                            cityname = "KAB FLORES TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5307",
                            citycode = "MME",
                            citymerger = "SIKKA",
                            cityname = "KAB SIKKA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5308",
                            citycode = "ENE",
                            citymerger = "ENDE",
                            cityname = "KAB ENDE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5309",
                            citycode = "BXD",
                            citymerger = "NGADA",
                            cityname = "KAB NGADA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5310",
                            citycode = "BJW",
                            citymerger = "MANGGARAI",
                            cityname = "KAB MANGGARAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5311",
                            citycode = "WGP",
                            citymerger = "SUMBA TIMUR",
                            cityname = "KAB SUMBA TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5312",
                            citycode = "TMC",
                            citymerger = "SUMBA BARAT",
                            cityname = "KAB SUMBA BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5313",
                            citycode = "LWA",
                            citymerger = "LEMBATA",
                            cityname = "KAB LEMBATA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5314",
                            citycode = "RTI",
                            citymerger = "ROTE NDAO",
                            cityname = "KAB ROTE NDAO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5315",
                            citycode = "LBJ",
                            citymerger = "MANGGARAI BARAT",
                            cityname = "KAB MANGGARAI BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5316",
                            citycode = "BXD",
                            citymerger = "NAGEKEO",
                            cityname = "KAB NAGEKEO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5317",
                            citycode = "TMC",
                            citymerger = "SUMBA TENGAH",
                            cityname = "KAB SUMBA TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5318",
                            citycode = "TMC",
                            citymerger = "SUMBA BARAT DAYA",
                            cityname = "KAB SUMBA BARAT DAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5319",
                            citycode = "BJW",
                            citymerger = "MANGGARAI TIMUR",
                            cityname = "KAB MANGGARAI TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5320",
                            citycode = "SEB",
                            citymerger = "SABU RAIJUA",
                            cityname = "KAB SABU RAIJUA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5321",
                            citycode = "KEF",
                            citymerger = "MALAKA",
                            cityname = "KAB MALAKA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "5371",
                            citycode = "KOE",
                            citymerger = "KUPANG",
                            cityname = "KOTA KUPANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KOE",
                            provid = "53"
                        },
                        new
                        {
                            cityid = "6101",
                            citycode = "SBS",
                            citymerger = "SAMBAS",
                            cityname = "KAB SAMBAS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6102",
                            citycode = "MPW",
                            citymerger = "MEMPAWAH",
                            cityname = "KAB MEMPAWAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6103",
                            citycode = "SGO",
                            citymerger = "SANGGAU",
                            cityname = "KAB SANGGAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6104",
                            citycode = "KTG",
                            citymerger = "KETAPANG",
                            cityname = "KAB KETAPANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6105",
                            citycode = "SQG",
                            citymerger = "SINTANG",
                            cityname = "KAB SINTANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6106",
                            citycode = "PTS",
                            citymerger = "KAPUAS HULU",
                            cityname = "KAB KAPUAS HULU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6107",
                            citycode = "BKY",
                            citymerger = "BENGKAYANG",
                            cityname = "KAB BENGKAYANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6108",
                            citycode = "LDK",
                            citymerger = "LANDAK",
                            cityname = "KAB LANDAK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6109",
                            citycode = "SKD",
                            citymerger = "SEKADAU",
                            cityname = "KAB SEKADAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6110",
                            citycode = "MLW",
                            citymerger = "MELAWI",
                            cityname = "KAB MELAWI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6111",
                            citycode = "KKU",
                            citymerger = "KAYONG UTARA",
                            cityname = "KAB KAYONG UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6112",
                            citycode = "KKR",
                            citymerger = "KUBU RAYA",
                            cityname = "KAB KUBU RAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6171",
                            citycode = "PNK",
                            citymerger = "PONTIANAK",
                            cityname = "KOTA PONTIANAK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6172",
                            citycode = "SKW",
                            citymerger = "SINGKAWANG",
                            cityname = "KOTA SINGKAWANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PNK",
                            provid = "61"
                        },
                        new
                        {
                            cityid = "6201",
                            citycode = "PBU",
                            citymerger = "KOTAWARINGIN BARAT",
                            cityname = "KAB KOTAWARINGIN BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PBU",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6202",
                            citycode = "SMQ",
                            citymerger = "KOTAWARINGIN TIMUR",
                            cityname = "KAB KOTAWARINGIN TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PBU",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6203",
                            citycode = "KPS",
                            citymerger = "KAPUAS",
                            cityname = "KAB KAPUAS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6204",
                            citycode = "BTS",
                            citymerger = "BARITO SELATAN",
                            cityname = "KAB BARITO SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6205",
                            citycode = "MTW",
                            citymerger = "BARITO UTARA",
                            cityname = "KAB BARITO UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6206",
                            citycode = "KTN",
                            citymerger = "KATINGAN",
                            cityname = "KAB KATINGAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6207",
                            citycode = "SRY",
                            citymerger = "SERUYAN",
                            cityname = "KAB SERUYAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6208",
                            citycode = "SRA",
                            citymerger = "SUKAMARA",
                            cityname = "KAB SUKAMARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PBU",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6209",
                            citycode = "LMD",
                            citymerger = "LAMANDAU",
                            cityname = "KAB LAMANDAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PBU",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6210",
                            citycode = "KRN",
                            citymerger = "GUNUNG MAS",
                            cityname = "KAB GUNUNG MAS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6211",
                            citycode = "PPS",
                            citymerger = "PULANG PISAU",
                            cityname = "KAB PULANG PISAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6212",
                            citycode = "PCH",
                            citymerger = "MURUNG RAYA",
                            cityname = "KAB MURUNG RAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6213",
                            citycode = "TMY",
                            citymerger = "BARITO TIMUR",
                            cityname = "KAB BARITO TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6271",
                            citycode = "PKY",
                            citymerger = "PALANGKARAYA",
                            cityname = "KOTA PALANGKARAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PKY",
                            provid = "62"
                        },
                        new
                        {
                            cityid = "6301",
                            citycode = "PLE",
                            citymerger = "TANAH LAUT",
                            cityname = "KAB TANAH LAUT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6302",
                            citycode = "KTB",
                            citymerger = "KOTABARU",
                            cityname = "KAB KOTABARU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6303",
                            citycode = "BJB",
                            citymerger = "BANJAR",
                            cityname = "KAB BANJAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6304",
                            citycode = "MRB",
                            citymerger = "BARITO KUALA",
                            cityname = "KAB BARITO KUALA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6305",
                            citycode = "KDG",
                            citymerger = "TAPIN",
                            cityname = "KAB TAPIN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6306",
                            citycode = "KDG",
                            citymerger = "HULU SUNGAI SELATAN",
                            cityname = "KAB HULU SUNGAI SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6307",
                            citycode = "KDG",
                            citymerger = "HULU SUNGAI TENGAH",
                            cityname = "KAB HULU SUNGAI TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6308",
                            citycode = "AMT",
                            citymerger = "HULU SUNGAI UTARA",
                            cityname = "KAB HULU SUNGAI UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6309",
                            citycode = "TNJ",
                            citymerger = "TABALONG",
                            cityname = "KAB TABALONG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6310",
                            citycode = "BTL",
                            citymerger = "TANAH BUMBU",
                            cityname = "KAB TANAH BUMBU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6311",
                            citycode = "TNJ",
                            citymerger = "BALANGAN",
                            cityname = "KAB BALANGAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6371",
                            citycode = "BDJ",
                            citymerger = "BANJARMASIN",
                            cityname = "KOTA BANJARMASIN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6372",
                            citycode = "BJB",
                            citymerger = "BANJARBARU",
                            cityname = "KOTA BANJARBARU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BDJ",
                            provid = "63"
                        },
                        new
                        {
                            cityid = "6401",
                            citycode = "TMB",
                            citymerger = "PASER",
                            cityname = "KAB PASER",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BPN",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6402",
                            citycode = "KOD",
                            citymerger = "KUTAI KARTANEGARA",
                            cityname = "KAB KUTAI KARTANEGARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRI",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6403",
                            citycode = "BEJ",
                            citymerger = "BERAU",
                            cityname = "KAB BERAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BPN",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6407",
                            citycode = "DTD",
                            citymerger = "KUTAI BARAT",
                            cityname = "KAB KUTAI BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRI",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6408",
                            citycode = "SGT",
                            citymerger = "KUTAI TIMUR",
                            cityname = "KAB KUTAI TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRI",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6409",
                            citycode = "PPU",
                            citymerger = "PENAJAM PASER UTARA",
                            cityname = "KAB PENAJAM PASER UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BPN",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6411",
                            citycode = "SRI",
                            citymerger = "MAHAKAM ULU",
                            cityname = "KAB MAHAKAM ULU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRI",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6471",
                            citycode = "BPN",
                            citymerger = "BALIKPAPAN",
                            cityname = "KOTA BALIKPAPAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BPN",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6472",
                            citycode = "SRI",
                            citymerger = "SAMARINDA",
                            cityname = "KOTA SAMARINDA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRI",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6474",
                            citycode = "BXT",
                            citymerger = "BONTANG",
                            cityname = "KOTA BONTANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SRI",
                            provid = "64"
                        },
                        new
                        {
                            cityid = "6501",
                            citycode = "TRK",
                            citymerger = "BULUNGAN",
                            cityname = "KAB BULUNGAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TRK",
                            provid = "65"
                        },
                        new
                        {
                            cityid = "6502",
                            citycode = "TRK",
                            citymerger = "MALINAU",
                            cityname = "KAB MALINAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TRK",
                            provid = "65"
                        },
                        new
                        {
                            cityid = "6503",
                            citycode = "TRK",
                            citymerger = "NUNUKAN",
                            cityname = "KAB NUNUKAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TRK",
                            provid = "65"
                        },
                        new
                        {
                            cityid = "6504",
                            citycode = "TRK",
                            citymerger = "TANA TIDUNG",
                            cityname = "KAB TANA TIDUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TRK",
                            provid = "65"
                        },
                        new
                        {
                            cityid = "6571",
                            citycode = "TRK",
                            citymerger = "TARAKAN",
                            cityname = "KOTA TARAKAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TRK",
                            provid = "65"
                        },
                        new
                        {
                            cityid = "7101",
                            citycode = "KTO",
                            citymerger = "BOLAANG MONGONDOW",
                            cityname = "KAB BOLAANG MONGONDOW",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7102",
                            citycode = "TDO",
                            citymerger = "MINAHASA",
                            cityname = "KAB MINAHASA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7103",
                            citycode = "THU",
                            citymerger = "KEPULAUAN SANGIHE",
                            cityname = "KAB KEPULAUAN SANGIHE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7104",
                            citycode = "TLD",
                            citymerger = "KEPULAUAN TALAUD",
                            cityname = "KAB KEPULAUAN TALAUD",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7105",
                            citycode = "AMR",
                            citymerger = "MINAHASA SELATAN",
                            cityname = "KAB MINAHASA SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7106",
                            citycode = "ARM",
                            citymerger = "MINAHASA UTARA",
                            cityname = "KAB MINAHASA UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7107",
                            citycode = "RTH",
                            citymerger = "MINAHASA TENGGARA",
                            cityname = "KAB MINAHASA TENGGARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7108",
                            citycode = "BMU",
                            citymerger = "BOLAANG MONGONDOW UTARA",
                            cityname = "KAB BOLAANG MONGONDOW UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7109",
                            citycode = "STR",
                            citymerger = "KEP SIAU",
                            cityname = "KAB KEP SIAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7110",
                            citycode = "BMT",
                            citymerger = "BOLAANG MONGONDOW TIMUR",
                            cityname = "KAB BOLAANG MONGONDOW TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7111",
                            citycode = "BMS",
                            citymerger = "BOLAANG MONGONDOW SELATAN",
                            cityname = "KAB BOLAANG MONGONDOW SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7171",
                            citycode = "MDC",
                            citymerger = "MANADO",
                            cityname = "KOTA MANADO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7172",
                            citycode = "BTU",
                            citymerger = "BITUNG",
                            cityname = "KOTA BITUNG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7173",
                            citycode = "TMH",
                            citymerger = "TOMOHON",
                            cityname = "KOTA TOMOHON",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7174",
                            citycode = "KTO",
                            citymerger = "KOTAMOBAGU",
                            cityname = "KOTA KOTAMOBAGU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "71"
                        },
                        new
                        {
                            cityid = "7201",
                            citycode = "BGI",
                            citymerger = "BANGGAI",
                            cityname = "KAB BANGGAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7202",
                            citycode = "PSJ",
                            citymerger = "POSO",
                            cityname = "KAB POSO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7203",
                            citycode = "DGL",
                            citymerger = "DONGGALA",
                            cityname = "KAB DONGGALA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7204",
                            citycode = "TOL",
                            citymerger = "TOLI TOLI",
                            cityname = "KAB TOLI TOLI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7205",
                            citycode = "BUO",
                            citymerger = "BUOL",
                            cityname = "KAB BUOL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7206",
                            citycode = "MOR",
                            citymerger = "MOROWALI",
                            cityname = "KAB MOROWALI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7207",
                            citycode = "BGK",
                            citymerger = "BANGGAI KEPULAUAN",
                            cityname = "KAB BANGGAI KEPULAUAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7208",
                            citycode = "PGM",
                            citymerger = "PARIGI MOUTONG",
                            cityname = "KAB PARIGI MOUTONG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7209",
                            citycode = "TJU",
                            citymerger = "TOJO UNA UNA",
                            cityname = "KAB TOJO UNA UNA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7210",
                            citycode = "SIM",
                            citymerger = "SIGI",
                            cityname = "KAB SIGI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7211",
                            citycode = "BGL",
                            citymerger = "BANGGAI LAUT",
                            cityname = "KAB BANGGAI LAUT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7212",
                            citycode = "MRU",
                            citymerger = "MOROWALI UTARA",
                            cityname = "KAB MOROWALI UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7271",
                            citycode = "PLW",
                            citymerger = "PALU",
                            cityname = "KOTA PALU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "72"
                        },
                        new
                        {
                            cityid = "7301",
                            citycode = "SLY",
                            citymerger = "KEPULAUAN SELAYAR",
                            cityname = "KAB KEPULAUAN SELAYAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7302",
                            citycode = "BUL",
                            citymerger = "BULUKUMBA",
                            cityname = "KAB BULUKUMBA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7303",
                            citycode = "UPG",
                            citymerger = "BANTAENG",
                            cityname = "KAB BANTAENG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7304",
                            citycode = "UPG",
                            citymerger = "JENEPONTO",
                            cityname = "KAB JENEPONTO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7305",
                            citycode = "UPG",
                            citymerger = "TAKALAR",
                            cityname = "KAB TAKALAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7306",
                            citycode = "UPG",
                            citymerger = "GOWA",
                            cityname = "KAB GOWA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7307",
                            citycode = "BUL",
                            citymerger = "SINJAI",
                            cityname = "KAB SINJAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7308",
                            citycode = "BNE",
                            citymerger = "BONE",
                            cityname = "KAB BONE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7309",
                            citycode = "MRS",
                            citymerger = "MAROS",
                            cityname = "KAB MAROS",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7310",
                            citycode = "PKP",
                            citymerger = "PANGKAJENE",
                            cityname = "KAB PANGKAJENE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7311",
                            citycode = "BRU",
                            citymerger = "BARRU",
                            cityname = "KAB BARRU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7312",
                            citycode = "SOP",
                            citymerger = "SOPPENG",
                            cityname = "KAB SOPPENG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7313",
                            citycode = "SKG",
                            citymerger = "WAJO",
                            cityname = "KAB WAJO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7314",
                            citycode = "SDR",
                            citymerger = "SIDENRENG RAPPANG",
                            cityname = "KAB SIDENRENG RAPPANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7315",
                            citycode = "PIN",
                            citymerger = "PINRANG",
                            cityname = "KAB PINRANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7316",
                            citycode = "EKG",
                            citymerger = "ENREKANG",
                            cityname = "KAB ENREKANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7317",
                            citycode = "LUW",
                            citymerger = "LUWU",
                            cityname = "KAB LUWU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7318",
                            citycode = "MKL",
                            citymerger = "TANA TORAJA",
                            cityname = "KAB TANA TORAJA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7322",
                            citycode = "LUT",
                            citymerger = "LUWU UTARA",
                            cityname = "KAB LUWU UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7324",
                            citycode = "LUM",
                            citymerger = "LUWU TIMUR",
                            cityname = "KAB LUWU TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7326",
                            citycode = "RTP",
                            citymerger = "TORAJA UTARA",
                            cityname = "KAB TORAJA UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7371",
                            citycode = "UPG",
                            citymerger = "MAKASSAR",
                            cityname = "KOTA MAKASSAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7372",
                            citycode = "PRE",
                            citymerger = "PARE PARE",
                            cityname = "KOTA PARE PARE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7373",
                            citycode = "PLP",
                            citymerger = "PALOPO",
                            cityname = "KOTA PALOPO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "73"
                        },
                        new
                        {
                            cityid = "7401",
                            citycode = "KKA",
                            citymerger = "KOLAKA",
                            cityname = "KAB KOLAKA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7402",
                            citycode = "UNH",
                            citymerger = "KONAWE",
                            cityname = "KAB KONAWE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7403",
                            citycode = "RAH",
                            citymerger = "MUNA",
                            cityname = "KAB MUNA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7404",
                            citycode = "BUW",
                            citymerger = "BUTON",
                            cityname = "KAB BUTON",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7405",
                            citycode = "UNH",
                            citymerger = "KONAWE SELATAN",
                            cityname = "KAB KONAWE SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7406",
                            citycode = "RMB",
                            citymerger = "BOMBANA",
                            cityname = "KAB BOMBANA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7407",
                            citycode = "WKB",
                            citymerger = "WAKATOBI",
                            cityname = "KAB WAKATOBI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7408",
                            citycode = "KKA",
                            citymerger = "KOLAKA UTARA",
                            cityname = "KAB KOLAKA UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7409",
                            citycode = "UNH",
                            citymerger = "KONAWE UTARA",
                            cityname = "KAB KONAWE UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7410",
                            citycode = "BUW",
                            citymerger = "BUTON UTARA",
                            cityname = "KAB BUTON UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7411",
                            citycode = "KKA",
                            citymerger = "KOLAKA TIMUR",
                            cityname = "KAB KOLAKA TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7412",
                            citycode = "UNH",
                            citymerger = "KONAWE KEPULAUAN",
                            cityname = "KAB KONAWE KEPULAUAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7413",
                            citycode = "RAH",
                            citymerger = "MUNA BARAT",
                            cityname = "KAB MUNA BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7414",
                            citycode = "BUW",
                            citymerger = "BUTON TENGAH",
                            cityname = "KAB BUTON TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7415",
                            citycode = "BUW",
                            citymerger = "BUTON SELATAN",
                            cityname = "KAB BUTON SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7471",
                            citycode = "KDI",
                            citymerger = "KENDARI",
                            cityname = "KOTA KENDARI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7472",
                            citycode = "BUW",
                            citymerger = "BAU BAU",
                            cityname = "KOTA BAU BAU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "KDI",
                            provid = "74"
                        },
                        new
                        {
                            cityid = "7501",
                            citycode = "GTL",
                            citymerger = "GORONTALO",
                            cityname = "KAB GORONTALO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "GTO",
                            provid = "75"
                        },
                        new
                        {
                            cityid = "7502",
                            citycode = "BLM",
                            citymerger = "BOALEMO",
                            cityname = "KAB BOALEMO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "GTO",
                            provid = "75"
                        },
                        new
                        {
                            cityid = "7503",
                            citycode = "BLO",
                            citymerger = "BONE BOLANGO",
                            cityname = "KAB BONE BOLANGO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "GTO",
                            provid = "75"
                        },
                        new
                        {
                            cityid = "7504",
                            citycode = "PWH",
                            citymerger = "PAHUWATO",
                            cityname = "KAB PAHUWATO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "GTO",
                            provid = "75"
                        },
                        new
                        {
                            cityid = "7505",
                            citycode = "LBT",
                            citymerger = "GORONTALO UTARA",
                            cityname = "KAB GORONTALO UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "GTO",
                            provid = "75"
                        },
                        new
                        {
                            cityid = "7571",
                            citycode = "GTO",
                            citymerger = "GORONTALO",
                            cityname = "KOTA GORONTALO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "GTO",
                            provid = "75"
                        },
                        new
                        {
                            cityid = "7601",
                            citycode = "MJU",
                            citymerger = "MAMUJU UTARA",
                            cityname = "KAB MAMUJU UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "PLW",
                            provid = "76"
                        },
                        new
                        {
                            cityid = "7602",
                            citycode = "MMJ",
                            citymerger = "MAMUJU",
                            cityname = "KAB MAMUJU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "76"
                        },
                        new
                        {
                            cityid = "7603",
                            citycode = "MMS",
                            citymerger = "MAMASA",
                            cityname = "KAB MAMASA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "76"
                        },
                        new
                        {
                            cityid = "7604",
                            citycode = "POL",
                            citymerger = "POLEWALI MANDAR",
                            cityname = "KAB POLEWALI MANDAR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "76"
                        },
                        new
                        {
                            cityid = "7605",
                            citycode = "MJN",
                            citymerger = "MAJENE",
                            cityname = "KAB MAJENE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "76"
                        },
                        new
                        {
                            cityid = "7606",
                            citycode = "MJT",
                            citymerger = "MAMUJU TENGAH",
                            cityname = "KAB MAMUJU TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "UPG",
                            provid = "76"
                        },
                        new
                        {
                            cityid = "8101",
                            citycode = "AHI",
                            citymerger = "MALUKU TENGAH",
                            cityname = "KAB MALUKU TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8102",
                            citycode = "LUV",
                            citymerger = "MALUKU TENGGARA",
                            cityname = "KAB MALUKU TENGGARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8103",
                            citycode = "SXK",
                            citymerger = "MALUKU TENGGARA BARAT",
                            cityname = "KAB MALUKU TENGGARA BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8104",
                            citycode = "NAM",
                            citymerger = "BURU",
                            cityname = "KAB BURU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8105",
                            citycode = "BUL",
                            citymerger = "SERAM BAGIAN TIMUR",
                            cityname = "KAB SERAM BAGIAN TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8106",
                            citycode = "WHI",
                            citymerger = "SERAM BAGIAN BARAT",
                            cityname = "KAB SERAM BAGIAN BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8107",
                            citycode = "DOB",
                            citymerger = "KEPULAUAN ARU",
                            cityname = "KAB KEPULAUAN ARU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8108",
                            citycode = "AMQ",
                            citymerger = "MALUKU BARAT DAYA",
                            cityname = "KAB MALUKU BARAT DAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8109",
                            citycode = "NRE",
                            citymerger = "BURU SELATAN",
                            cityname = "KAB BURU SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8171",
                            citycode = "AMQ",
                            citymerger = "AMBON",
                            cityname = "KOTA AMBON",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8172",
                            citycode = "LUV",
                            citymerger = "TUAL",
                            cityname = "KOTA TUAL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "AMQ",
                            provid = "81"
                        },
                        new
                        {
                            cityid = "8201",
                            citycode = "BCN",
                            citymerger = "HALMAHERA BARAT",
                            cityname = "KAB HALMAHERA BARAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8202",
                            citycode = "SSP",
                            citymerger = "HALMAHERA TENGAH",
                            cityname = "KAB HALMAHERA TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8203",
                            citycode = "TBL",
                            citymerger = "HALMAHERA UTARA",
                            cityname = "KAB HALMAHERA UTARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8204",
                            citycode = "WDA",
                            citymerger = "HALMAHERA SELATAN",
                            cityname = "KAB HALMAHERA SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8205",
                            citycode = "SLA",
                            citymerger = "KEPULAUAN SULA",
                            cityname = "KAB KEPULAUAN SULA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8206",
                            citycode = "BLI",
                            citymerger = "HALMAHERA TIMUR",
                            cityname = "KAB HALMAHERA TIMUR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8207",
                            citycode = "MRT",
                            citymerger = "PULAU MOROTAI",
                            cityname = "KAB PULAU MOROTAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8208",
                            citycode = "TLB",
                            citymerger = "PULAU TALIABU",
                            cityname = "KAB PULAU TALIABU",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8271",
                            citycode = "TTE",
                            citymerger = "TERNATE",
                            cityname = "KOTA TERNATE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TTE",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "8272",
                            citycode = "TDR",
                            citymerger = "TIDORE KEPULAUAN",
                            cityname = "KOTA TIDORE KEPULAUAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MDC",
                            provid = "82"
                        },
                        new
                        {
                            cityid = "9101",
                            citycode = "MKQ",
                            citymerger = "MERAUKE",
                            cityname = "KAB MERAUKE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MKQ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9102",
                            citycode = "WMX",
                            citymerger = "JAYAWIJAYA",
                            cityname = "KAB JAYAWIJAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9103",
                            citycode = "DJJ",
                            citymerger = "JAYAPURA",
                            cityname = "KAB JAYAPURA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9104",
                            citycode = "NBX",
                            citymerger = "NABIRE",
                            cityname = "KAB NABIRE",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BIK",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9105",
                            citycode = "ZRI",
                            citymerger = "KEPULAUAN YAPEN",
                            cityname = "KAB KEPULAUAN YAPEN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BIK",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9106",
                            citycode = "BIK",
                            citymerger = "BIAK NUMFOR",
                            cityname = "KAB BIAK NUMFOR",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BIK",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9107",
                            citycode = "ELR",
                            citymerger = "PUNCAK JAYA",
                            cityname = "KAB PUNCAK JAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9108",
                            citycode = "ILA",
                            citymerger = "PANIAI",
                            cityname = "KAB PANIAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BIK",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9109",
                            citycode = "TIM",
                            citymerger = "MIMIKA",
                            cityname = "KAB MIMIKA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "TIM",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9110",
                            citycode = "ZRM",
                            citymerger = "SARMI",
                            cityname = "KAB SARMI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9111",
                            citycode = "WAR",
                            citymerger = "KEEROM",
                            cityname = "KAB KEEROM",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9112",
                            citycode = "OKL",
                            citymerger = "PEGUNUNGAN BINTANG",
                            cityname = "KAB PEGUNUNGAN BINTANG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9113",
                            citycode = "DEK",
                            citymerger = "YAHUKIMO",
                            cityname = "KAB YAHUKIMO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9114",
                            citycode = "KBF",
                            citymerger = "TOLIKARA",
                            cityname = "KAB TOLIKARA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9115",
                            citycode = "BTW",
                            citymerger = "WAROPEN",
                            cityname = "KAB WAROPEN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BIK",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9116",
                            citycode = "MDP",
                            citymerger = "BOVEN DIGOEL",
                            cityname = "KAB BOVEN DIGOEL",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MKQ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9117",
                            citycode = "KEI",
                            citymerger = "MAPPI",
                            cityname = "KAB MAPPI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MKQ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9118",
                            citycode = "AGT",
                            citymerger = "ASMAT",
                            cityname = "KAB ASMAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MKQ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9119",
                            citycode = "SRW",
                            citymerger = "SUPIORI",
                            cityname = "KAB SUPIORI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BIK",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9120",
                            citycode = "OKL",
                            citymerger = "MAMBERAMO RAYA",
                            cityname = "KAB MAMBERAMO RAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9121",
                            citycode = "OKL",
                            citymerger = "MAMBERAMO TENGAH",
                            cityname = "KAB MAMBERAMO TENGAH",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9122",
                            citycode = "ELM",
                            citymerger = "YALIMO",
                            cityname = "KAB YALIMO",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9123",
                            citycode = "TOM",
                            citymerger = "LANNY JAYA",
                            cityname = "KAB LANNY JAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9124",
                            citycode = "KYM",
                            citymerger = "NDUGA",
                            cityname = "KAB NDUGA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9125",
                            citycode = "ILA",
                            citymerger = "PUNCAK",
                            cityname = "KAB PUNCAK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9126",
                            citycode = "NBX",
                            citymerger = "DOGIYAI",
                            cityname = "KAB DOGIYAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9127",
                            citycode = "ZGP",
                            citymerger = "INTAN JAYA",
                            cityname = "KAB INTAN JAYA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9128",
                            citycode = "TIG",
                            citymerger = "DEIYAI",
                            cityname = "KAB DEIYAI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "BIK",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9171",
                            citycode = "DJJ",
                            citymerger = "JAYAPURA",
                            cityname = "KOTA JAYAPURA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "91"
                        },
                        new
                        {
                            cityid = "9201",
                            citycode = "SOQ",
                            citymerger = "SORONG",
                            cityname = "KAB SORONG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9202",
                            citycode = "MKW",
                            citymerger = "MANOKWARI",
                            cityname = "KAB MANOKWARI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MKW",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9203",
                            citycode = "FFK",
                            citymerger = "FAK FAK",
                            cityname = "KAB FAK FAK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9204",
                            citycode = "SOQ",
                            citymerger = "SORONG SELATAN",
                            cityname = "KAB SORONG SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9205",
                            citycode = "GAV",
                            citymerger = "RAJA AMPAT",
                            cityname = "KAB RAJA AMPAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9206",
                            citycode = "NTI",
                            citymerger = "TELUK BINTUNI",
                            cityname = "KAB TELUK BINTUNI",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9207",
                            citycode = "RAS",
                            citymerger = "TELUK WONDAMA",
                            cityname = "KAB TELUK WONDAMA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MKW",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9208",
                            citycode = "KNG",
                            citymerger = "KAIMANA",
                            cityname = "KAB KAIMANA",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9209",
                            citycode = "FEF",
                            citymerger = "TAMBRAUW",
                            cityname = "KAB TAMBRAUW",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9210",
                            citycode = "AFT",
                            citymerger = "MAYBRAT",
                            cityname = "KAB MAYBRAT",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9211",
                            citycode = "MKW",
                            citymerger = "MANOKWARI SELATAN",
                            cityname = "KAB MANOKWARI SELATAN",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "MKW",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9212",
                            citycode = "MKW",
                            citymerger = "PEGUNUNGAN ARFAK",
                            cityname = "KAB PEGUNUNGAN ARFAK",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "DJJ",
                            provid = "92"
                        },
                        new
                        {
                            cityid = "9271",
                            citycode = "SOQ",
                            citymerger = "SORONG",
                            cityname = "KOTA SORONG",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            hubcode = "SOQ",
                            provid = "92"
                        });
                });

            modelBuilder.Entity("iDss.X.Models.CityIntl", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("airport")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("citycode")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("cityname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("countrycode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<decimal?>("disc3pl1")
                        .HasColumnType("decimal(10,2)");

                    b.Property<decimal?>("disc3pl2")
                        .HasColumnType("decimal(10,2)");

                    b.Property<decimal?>("disc3pl3")
                        .HasColumnType("decimal(10,2)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("region")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("zone3pl1")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("zone3pl2")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("zone3pl3")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("id");

                    b.HasIndex("countrycode");

                    b.ToTable("mdt_cityintl");
                });

            modelBuilder.Entity("iDss.X.Models.CityZone", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("cityid")
                        .IsRequired()
                        .HasMaxLength(4)
                        .HasColumnType("nvarchar(4)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("zoneid")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.HasKey("id");

                    b.HasIndex("cityid");

                    b.HasIndex("zoneid");

                    b.ToTable("mdt_cityzone");
                });

            modelBuilder.Entity("iDss.X.Models.CneeDirectory", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr3")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("attname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("cnelat")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("cnelong")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("cnename")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("costcenter")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("countrycode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("descode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<string>("email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("postcode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("id");

                    b.HasIndex("distid");

                    b.ToTable("trx_cneedirectory");
                });

            modelBuilder.Entity("iDss.X.Models.ConsigneeDetail", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<string>("acctno")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr3")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("attname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int?>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("cnelat")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("cnelong")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("cnename")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("costcenter")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("countrycode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("descode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<string>("email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("postcode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("id");

                    b.HasIndex("acctno");

                    b.HasIndex("awb")
                        .IsUnique();

                    b.HasIndex("branchid");

                    b.HasIndex("distid");

                    b.ToTable("trx_consignee");
                });

            modelBuilder.Entity("iDss.X.Models.CostComponent", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("componentname")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("description")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<int>("issystem")
                        .HasColumnType("int");

                    b.Property<string>("type")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.HasKey("id");

                    b.ToTable("mdt_costcomponent");
                });

            modelBuilder.Entity("iDss.X.Models.CostItem", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("componetname")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<decimal>("credit")
                        .HasColumnType("decimal(18,0)");

                    b.Property<decimal>("debit")
                        .HasColumnType("decimal(18,0)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("id");

                    b.ToTable("trx_costitem");
                });

            modelBuilder.Entity("iDss.X.Models.Counter", b =>
                {
                    b.Property<int>("counterid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("counterid"));

                    b.Property<string>("addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr3")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("citycode")
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("comission")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("countercode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("countername")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("latitude")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("longitude")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("phonealt")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("picname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("picno")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("postcode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("villid")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("counterid");

                    b.HasIndex("branchid");

                    b.HasIndex("villid");

                    b.ToTable("mdt_counter");
                });

            modelBuilder.Entity("iDss.X.Models.Country", b =>
                {
                    b.Property<string>("countrycode")
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("countryname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("isoalpha3")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("phonecode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.HasKey("countrycode");

                    b.ToTable("mdt_country");
                });

            modelBuilder.Entity("iDss.X.Models.Courier", b =>
                {
                    b.Property<string>("nip")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("couriercode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("couriername")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<int?>("freqdelivery")
                        .HasColumnType("int");

                    b.Property<int?>("fuelquota")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int?>("status")
                        .HasColumnType("int");

                    b.HasKey("nip");

                    b.HasIndex("branchid");

                    b.HasIndex("couriercode")
                        .IsUnique();

                    b.HasIndex("distid");

                    b.ToTable("mdt_courier");
                });

            modelBuilder.Entity("iDss.X.Models.District", b =>
                {
                    b.Property<string>("distid")
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<string>("cityid")
                        .IsRequired()
                        .HasMaxLength(4)
                        .HasColumnType("nvarchar(4)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("distid");

                    b.HasIndex("cityid");

                    b.ToTable("mdt_district");

                    b.HasData(
                        new
                        {
                            distid = "317307",
                            cityid = "3173",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            distname = "Pal Merah",
                            flag = 1
                        });
                });

            modelBuilder.Entity("iDss.X.Models.Industry", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("industryname")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.ToTable("mdt_industry");
                });

            modelBuilder.Entity("iDss.X.Models.NCSBankAccount", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("accountname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("accountno")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("bankcode")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("bankname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.ToTable("mdt_ncsbankaccount");
                });

            modelBuilder.Entity("iDss.X.Models.PackingPrice", b =>
                {
                    b.Property<string>("packingcode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("sizecode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<decimal>("price")
                        .HasColumnType("decimal(10,0)");

                    b.HasKey("packingcode", "sizecode");

                    b.HasIndex("sizecode");

                    b.ToTable("mdt_packingprice");
                });

            modelBuilder.Entity("iDss.X.Models.PackingSize", b =>
                {
                    b.Property<string>("sizecode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("sizename")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.HasKey("sizecode");

                    b.ToTable("mdt_packingsize");

                    b.HasData(
                        new
                        {
                            sizecode = "s",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            remarks = "",
                            sizename = "Small"
                        },
                        new
                        {
                            sizecode = "m",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            remarks = "",
                            sizename = "Medium"
                        },
                        new
                        {
                            sizecode = "l",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            remarks = "",
                            sizename = "Large"
                        });
                });

            modelBuilder.Entity("iDss.X.Models.PackingType", b =>
                {
                    b.Property<string>("packingcode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("packingname")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("packingcode");

                    b.ToTable("mdt_packingtype");

                    b.HasData(
                        new
                        {
                            packingcode = "101",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            packingname = "Bubble Wrap",
                            remarks = ""
                        },
                        new
                        {
                            packingcode = "102",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            packingname = "Kardus (Box)",
                            remarks = ""
                        },
                        new
                        {
                            packingcode = "103",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            packingname = "Kayu",
                            remarks = ""
                        },
                        new
                        {
                            packingcode = "104",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            packingname = "Styrofoam",
                            remarks = ""
                        });
                });

            modelBuilder.Entity("iDss.X.Models.PaymentDetail", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("paymentgateway")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("paymentstatus")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("paymenttype")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.HasKey("id");

                    b.HasIndex("awb")
                        .IsUnique();

                    b.ToTable("trx_payment");
                });

            modelBuilder.Entity("iDss.X.Models.PickupRegular", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("acctname")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("acctno")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("addr")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<int?>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("couriercode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("couriername")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<int>("isautodispatch")
                        .HasColumnType("int");

                    b.Property<string>("latitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("longitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("notes")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("picemail")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("pickuppoint")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("pickuptype")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("picname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("picphone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("postcode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("transporttype")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("id");

                    b.HasIndex("acctno");

                    b.HasIndex("branchid");

                    b.HasIndex("couriercode");

                    b.HasIndex("distid");

                    b.ToTable("pum_pickupregular");
                });

            modelBuilder.Entity("iDss.X.Models.PickupRequest", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("acctname")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("acctno")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("addr")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("apireqid")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int?>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("branchname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("couriercode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("couriername")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<string>("email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("latitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("longitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("notes")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<DateOnly>("pickupdate")
                        .HasColumnType("date");

                    b.Property<string>("pickupno")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("pickuppoint")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("pickuptype")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("picname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("postcode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<TimeOnly>("timefrom")
                        .HasColumnType("time");

                    b.Property<TimeOnly>("timeto")
                        .HasColumnType("time");

                    b.Property<string>("transporttype")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("id");

                    b.HasIndex("acctno");

                    b.HasIndex("branchid");

                    b.HasIndex("couriercode");

                    b.HasIndex("distid");

                    b.HasIndex("pickupno")
                        .IsUnique();

                    b.ToTable("pum_pickuprequest");
                });

            modelBuilder.Entity("iDss.X.Models.PickupSchedule", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("pickupday")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<int>("puregid")
                        .HasColumnType("int");

                    b.Property<int>("shift")
                        .HasColumnType("int");

                    b.Property<TimeOnly>("timefrom")
                        .HasColumnType("time");

                    b.Property<TimeOnly>("timeto")
                        .HasColumnType("time");

                    b.HasKey("id");

                    b.HasIndex("puregid");

                    b.ToTable("pum_pickupschedule");
                });

            modelBuilder.Entity("iDss.X.Models.PickupStatusPool", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("pickupno")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("pickupstatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.HasIndex("pickupno");

                    b.ToTable("pum_pickupstatuspool");
                });

            modelBuilder.Entity("iDss.X.Models.PostCode", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("postcode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("villid")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("id");

                    b.HasIndex("villid");

                    b.ToTable("mdt_postcode");
                });

            modelBuilder.Entity("iDss.X.Models.Province", b =>
                {
                    b.Property<string>("provid")
                        .HasMaxLength(2)
                        .HasColumnType("nvarchar(2)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("provname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("provid");

                    b.ToTable("mdt_province");

                    b.HasData(
                        new
                        {
                            provid = "11",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "ACEH"
                        },
                        new
                        {
                            provid = "12",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "SUMATERA UTARA"
                        },
                        new
                        {
                            provid = "13",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "SUMATERA BARAT"
                        },
                        new
                        {
                            provid = "14",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "RIAU"
                        },
                        new
                        {
                            provid = "15",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "JAMBI"
                        },
                        new
                        {
                            provid = "16",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "SUMATERA SELATAN"
                        },
                        new
                        {
                            provid = "17",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "BENGKULU"
                        },
                        new
                        {
                            provid = "18",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "LAMPUNG"
                        },
                        new
                        {
                            provid = "19",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "BANGKA BELITUNG"
                        },
                        new
                        {
                            provid = "21",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "KEPULAUAN RIAU"
                        },
                        new
                        {
                            provid = "31",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "DKI JAKARTA"
                        },
                        new
                        {
                            provid = "32",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "JAWA BARAT"
                        },
                        new
                        {
                            provid = "33",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "JAWA TENGAH"
                        },
                        new
                        {
                            provid = "34",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "YOGYAKARTA"
                        },
                        new
                        {
                            provid = "35",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "JAWA TIMUR"
                        },
                        new
                        {
                            provid = "36",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "BANTEN"
                        },
                        new
                        {
                            provid = "51",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "BALI"
                        },
                        new
                        {
                            provid = "52",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "NUSA TENGGARA BARAT"
                        },
                        new
                        {
                            provid = "53",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "NUSA TENGGARA TIMUR"
                        },
                        new
                        {
                            provid = "61",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "KALIMANTAN BARAT"
                        },
                        new
                        {
                            provid = "62",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "KALIMANTAN TENGAH"
                        },
                        new
                        {
                            provid = "63",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "KALIMANTAN SELATAN"
                        },
                        new
                        {
                            provid = "64",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "KALIMANTAN TIMUR"
                        },
                        new
                        {
                            provid = "65",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "KALIMANTAN UTARA"
                        },
                        new
                        {
                            provid = "71",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "SULAWESI UTARA"
                        },
                        new
                        {
                            provid = "72",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "SULAWESI TENGAH"
                        },
                        new
                        {
                            provid = "73",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "SULAWESI SELATAN"
                        },
                        new
                        {
                            provid = "74",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "SULAWESI TENGGARA"
                        },
                        new
                        {
                            provid = "75",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "GORONTALO"
                        },
                        new
                        {
                            provid = "76",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "SULAWESI BARAT"
                        },
                        new
                        {
                            provid = "81",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "MALUKU"
                        },
                        new
                        {
                            provid = "82",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "MALUKU UTARA"
                        },
                        new
                        {
                            provid = "91",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "PAPUA"
                        },
                        new
                        {
                            provid = "92",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            flag = 1,
                            provname = "PAPUA BARAT"
                        });
                });

            modelBuilder.Entity("iDss.X.Models.ReasonPickup", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("reasoncode")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("reasongroup")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("reasonname")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.ToTable("mdt_reasonpickup");
                });

            modelBuilder.Entity("iDss.X.Models.ReasonUN", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("reasoncode")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("reasonname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.ToTable("mdt_reasonun");
                });

            modelBuilder.Entity("iDss.X.Models.ReassignPickup", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("pickupno")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("pickupstatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("prevcourier")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("reasonid")
                        .HasColumnType("int");

                    b.Property<string>("reasonname")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.HasIndex("pickupno");

                    b.ToTable("pum_reassignpickup");
                });

            modelBuilder.Entity("iDss.X.Models.Relation", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("relationcode")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("relationname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.ToTable("mdt_relation");
                });

            modelBuilder.Entity("iDss.X.Models.ReschedulePickup", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("pickupno")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("pickupstatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("reasonid")
                        .HasColumnType("int");

                    b.Property<string>("reasonname")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("requestor")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("id");

                    b.HasIndex("pickupno");

                    b.ToTable("pum_reschedulepickup");
                });

            modelBuilder.Entity("iDss.X.Models.Service", b =>
                {
                    b.Property<string>("servicecode")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("servicename")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.HasKey("servicecode");

                    b.ToTable("mdt_service");
                });

            modelBuilder.Entity("iDss.X.Models.ShipmentDetail", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<decimal>("actweight")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("apireqid")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("buyerorderno")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<decimal>("chargeweight")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("content")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("curr")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("deliveryitem")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("donumber")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("hubcode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("intruction")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("iscod")
                        .HasColumnType("int");

                    b.Property<int>("isedit")
                        .HasColumnType("int");

                    b.Property<int>("isinsurance")
                        .HasColumnType("int");

                    b.Property<int>("isnfd")
                        .HasColumnType("int");

                    b.Property<int>("isnotif")
                        .HasColumnType("int");

                    b.Property<int>("isprint")
                        .HasColumnType("int");

                    b.Property<int>("isrev")
                        .HasColumnType("int");

                    b.Property<decimal?>("itemvalue")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("linehaul")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("packingtype")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<DateOnly>("pickupdate")
                        .HasColumnType("date");

                    b.Property<string>("pickupno")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int>("pieces")
                        .HasColumnType("int");

                    b.Property<string>("refno")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("service")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("session")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("sla")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("trxtype")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("unitweight")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<decimal>("volweight")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("vouchercode")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("id");

                    b.HasIndex("awb")
                        .IsUnique();

                    b.HasIndex("pickupno");

                    b.ToTable("trx_shipmentdetail");
                });

            modelBuilder.Entity("iDss.X.Models.ShipperDetail", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<string>("acctno")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("addr3")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("attname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("costcenter")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("countrycode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<string>("email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("oricode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("postcode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("shippername")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("shplat")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("shplong")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.HasIndex("acctno");

                    b.HasIndex("awb")
                        .IsUnique();

                    b.HasIndex("branchid");

                    b.HasIndex("distid");

                    b.ToTable("trx_shipper");
                });

            modelBuilder.Entity("iDss.X.Models.SuccessPickup", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("attach1")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("attach2")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal>("cashreceived")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("latitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("longitude")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("pickupno")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("pickupstatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.HasIndex("pickupno");

                    b.ToTable("pum_successpickup");
                });

            modelBuilder.Entity("iDss.X.Models.TrxStaging", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<string>("acctno")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<decimal>("actweight")
                        .HasColumnType("decimal(10,2)");

                    b.Property<decimal>("addcost")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("apireqid")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("attach1")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("attach2")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("awb")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int?>("branchcne")
                        .HasColumnType("int");

                    b.Property<int>("branchori")
                        .HasColumnType("int");

                    b.Property<string>("buyerorderno")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("c_addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("c_addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("c_attname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("c_cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("c_countrycode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("c_distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<string>("c_email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("c_phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("c_postcode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("c_provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("chargeweight")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("cnelat")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("cnelong")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("cnename")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("codamount")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("codtype")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("content")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("costcenter")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("curr")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<decimal>("deliverycost")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("deliveryitem")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("descode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("donumber")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("hubcode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<decimal>("insurancecost")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("intruction")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("iscod")
                        .HasColumnType("int");

                    b.Property<int>("isdrop")
                        .HasColumnType("int");

                    b.Property<int>("isinsurance")
                        .HasColumnType("int");

                    b.Property<int>("isnfd")
                        .HasColumnType("int");

                    b.Property<int>("isnotif")
                        .HasColumnType("int");

                    b.Property<int>("isprod")
                        .HasColumnType("int");

                    b.Property<int>("isrev")
                        .HasColumnType("int");

                    b.Property<decimal?>("itemvalue")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("linehaul")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("oricode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<decimal>("packingcost")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("packingtype")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("paymentgateway")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("paymentstatus")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("paymenttype")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<DateOnly?>("pickupdate")
                        .HasColumnType("date");

                    b.Property<string>("pickupno")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int>("pieces")
                        .HasColumnType("int");

                    b.Property<string>("refno")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("refpayid")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("s_addr1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("s_addr2")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("s_attname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("s_cityname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("s_countrycode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("s_distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<string>("s_email")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("s_phone")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("s_postcode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("s_provname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("service")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("session")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("shippername")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("shplat")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("shplong")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal>("totaldiscount")
                        .HasColumnType("decimal(18,0)");

                    b.Property<string>("trxtype")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("unitweight")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<decimal>("volweight")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("vouchercode")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("id");

                    b.HasIndex("acctno");

                    b.HasIndex("branchcne");

                    b.HasIndex("branchori");

                    b.HasIndex("c_distid");

                    b.HasIndex("pickupno");

                    b.HasIndex("s_distid");

                    b.ToTable("trx_staging");
                });

            modelBuilder.Entity("iDss.X.Models.UnitItem", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<decimal>("actweight")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<decimal>("height")
                        .HasColumnType("decimal(10,0)");

                    b.Property<string>("itemcode")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("itemname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("length")
                        .HasColumnType("decimal(10,0)");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("volweight")
                        .HasColumnType("decimal(10,2)");

                    b.Property<decimal>("width")
                        .HasColumnType("decimal(10,0)");

                    b.HasKey("id");

                    b.ToTable("trx_unititem");
                });

            modelBuilder.Entity("iDss.X.Models.UnsuccessPickup", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"));

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<string>("pickupno")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("pickupstatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("reasonid")
                        .HasColumnType("int");

                    b.Property<string>("reasonname")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("remarks")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("id");

                    b.HasIndex("pickupno");

                    b.ToTable("pum_unsuccesspickup");
                });

            modelBuilder.Entity("iDss.X.Models.Village", b =>
                {
                    b.Property<string>("villid")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("distid")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("villname")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("villid");

                    b.HasIndex("distid");

                    b.ToTable("mdt_village");

                    b.HasData(
                        new
                        {
                            villid = "3173071006",
                            createdby = "System",
                            createddate = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            distid = "317307",
                            flag = 1,
                            villname = "Kota Bambu Selatan"
                        });
                });

            modelBuilder.Entity("iDss.X.Models.VoidTransaction", b =>
                {
                    b.Property<long>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("id"));

                    b.Property<int>("approval")
                        .HasColumnType("int");

                    b.Property<string>("approver")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("awb")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int>("branchid")
                        .HasColumnType("int");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("reasonvoid")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("remarks")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("requestpoint")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("id");

                    b.HasIndex("awb")
                        .IsUnique();

                    b.HasIndex("branchid");

                    b.ToTable("trx_void");
                });

            modelBuilder.Entity("iDss.X.Models.Zone", b =>
                {
                    b.Property<string>("zoneid")
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("createdby")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("createddate")
                        .HasColumnType("datetime2");

                    b.Property<int>("flag")
                        .HasColumnType("int");

                    b.Property<DateTime?>("modifieddate")
                        .HasColumnType("datetime2");

                    b.Property<string>("modifier")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("zonegroup")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("zonename")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("zoneid");

                    b.ToTable("mdt_zone");
                });

            modelBuilder.Entity("iDss.X.Models.AWBInventory", b =>
                {
                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Branch");
                });

            modelBuilder.Entity("iDss.X.Models.Account", b =>
                {
                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.CIF", "CIF")
                        .WithMany()
                        .HasForeignKey("cif")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Branch");

                    b.Navigation("CIF");
                });

            modelBuilder.Entity("iDss.X.Models.AccountAddr", b =>
                {
                    b.HasOne("iDss.X.Models.Account", "Account")
                        .WithMany("AccountAddrs")
                        .HasForeignKey("acctno")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.District", "District")
                        .WithMany()
                        .HasForeignKey("distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("District");
                });

            modelBuilder.Entity("iDss.X.Models.AccountCro", b =>
                {
                    b.HasOne("iDss.X.Models.Account", "Account")
                        .WithMany("AccountCros")
                        .HasForeignKey("acctno")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.CRO", "CRO")
                        .WithMany("AccountCros")
                        .HasForeignKey("crocode")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("CRO");
                });

            modelBuilder.Entity("iDss.X.Models.Agent", b =>
                {
                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Village", "Village")
                        .WithMany()
                        .HasForeignKey("villid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Branch");

                    b.Navigation("Village");
                });

            modelBuilder.Entity("iDss.X.Models.AppMenu", b =>
                {
                    b.HasOne("iDss.X.Models.AppModule", "Module")
                        .WithMany("Menus")
                        .HasForeignKey("moduleid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Module");
                });

            modelBuilder.Entity("iDss.X.Models.AppModule", b =>
                {
                    b.HasOne("iDss.X.Models.AppModuleCtg", "ModuleCtg")
                        .WithMany("Modules")
                        .HasForeignKey("modulectgid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ModuleCtg");
                });

            modelBuilder.Entity("iDss.X.Models.Branch", b =>
                {
                    b.HasOne("iDss.X.Models.Village", "Village")
                        .WithMany()
                        .HasForeignKey("villid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Village");
                });

            modelBuilder.Entity("iDss.X.Models.BranchAccount", b =>
                {
                    b.HasOne("iDss.X.Models.Account", "Account")
                        .WithMany()
                        .HasForeignKey("acctno")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("Branch");
                });

            modelBuilder.Entity("iDss.X.Models.CIF", b =>
                {
                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Industry", "Industry")
                        .WithMany("CIFs")
                        .HasForeignKey("industryid")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Branch");

                    b.Navigation("Industry");
                });

            modelBuilder.Entity("iDss.X.Models.CRO", b =>
                {
                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Branch");
                });

            modelBuilder.Entity("iDss.X.Models.CancelPickup", b =>
                {
                    b.HasOne("iDss.X.Models.PickupRequest", "PickupRequest")
                        .WithMany("CancelPickups")
                        .HasForeignKey("pickupno")
                        .HasPrincipalKey("pickupno")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PickupRequest");
                });

            modelBuilder.Entity("iDss.X.Models.CheckpointPool", b =>
                {
                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Courier", "Courier")
                        .WithMany("CheckpointPools")
                        .HasForeignKey("couriercode")
                        .HasPrincipalKey("couriercode")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.ReasonUN", "ReasonUN")
                        .WithMany()
                        .HasForeignKey("reasonid")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.Relation", "Relation")
                        .WithMany()
                        .HasForeignKey("relationid")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Branch");

                    b.Navigation("Courier");

                    b.Navigation("ReasonUN");

                    b.Navigation("Relation");
                });

            modelBuilder.Entity("iDss.X.Models.City", b =>
                {
                    b.HasOne("iDss.X.Models.Province", "Province")
                        .WithMany()
                        .HasForeignKey("provid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Province");
                });

            modelBuilder.Entity("iDss.X.Models.CityIntl", b =>
                {
                    b.HasOne("iDss.X.Models.Country", "Country")
                        .WithMany()
                        .HasForeignKey("countrycode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Country");
                });

            modelBuilder.Entity("iDss.X.Models.CityZone", b =>
                {
                    b.HasOne("iDss.X.Models.City", "City")
                        .WithMany()
                        .HasForeignKey("cityid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Zone", "Zone")
                        .WithMany()
                        .HasForeignKey("zoneid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("City");

                    b.Navigation("Zone");
                });

            modelBuilder.Entity("iDss.X.Models.CneeDirectory", b =>
                {
                    b.HasOne("iDss.X.Models.District", "District")
                        .WithMany()
                        .HasForeignKey("distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("District");
                });

            modelBuilder.Entity("iDss.X.Models.ConsigneeDetail", b =>
                {
                    b.HasOne("iDss.X.Models.Account", "Account")
                        .WithMany()
                        .HasForeignKey("acctno")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.ShipmentDetail", "ShipmentDetail")
                        .WithOne("ConsigneeDetail")
                        .HasForeignKey("iDss.X.Models.ConsigneeDetail", "awb")
                        .HasPrincipalKey("iDss.X.Models.ShipmentDetail", "awb")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.District", "District")
                        .WithMany()
                        .HasForeignKey("distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("Branch");

                    b.Navigation("District");

                    b.Navigation("ShipmentDetail");
                });

            modelBuilder.Entity("iDss.X.Models.Counter", b =>
                {
                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Village", "Village")
                        .WithMany()
                        .HasForeignKey("villid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Branch");

                    b.Navigation("Village");
                });

            modelBuilder.Entity("iDss.X.Models.Courier", b =>
                {
                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany("couriers")
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.District", "District")
                        .WithMany()
                        .HasForeignKey("distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Branch");

                    b.Navigation("District");
                });

            modelBuilder.Entity("iDss.X.Models.District", b =>
                {
                    b.HasOne("iDss.X.Models.City", "City")
                        .WithMany()
                        .HasForeignKey("cityid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("City");
                });

            modelBuilder.Entity("iDss.X.Models.PackingPrice", b =>
                {
                    b.HasOne("iDss.X.Models.PackingType", "PackingType")
                        .WithMany("PackingPrices")
                        .HasForeignKey("packingcode")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.PackingSize", "PackingSize")
                        .WithMany("PackingPrices")
                        .HasForeignKey("sizecode")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PackingSize");

                    b.Navigation("PackingType");
                });

            modelBuilder.Entity("iDss.X.Models.PaymentDetail", b =>
                {
                    b.HasOne("iDss.X.Models.ShipmentDetail", "ShipmentDetail")
                        .WithOne("PaymentDetail")
                        .HasForeignKey("iDss.X.Models.PaymentDetail", "awb")
                        .HasPrincipalKey("iDss.X.Models.ShipmentDetail", "awb")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ShipmentDetail");
                });

            modelBuilder.Entity("iDss.X.Models.PickupRegular", b =>
                {
                    b.HasOne("iDss.X.Models.Account", "Account")
                        .WithMany()
                        .HasForeignKey("acctno")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.Courier", "Courier")
                        .WithMany("PickupRegulars")
                        .HasForeignKey("couriercode")
                        .HasPrincipalKey("couriercode")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.District", "District")
                        .WithMany()
                        .HasForeignKey("distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("Branch");

                    b.Navigation("Courier");

                    b.Navigation("District");
                });

            modelBuilder.Entity("iDss.X.Models.PickupRequest", b =>
                {
                    b.HasOne("iDss.X.Models.Account", "Account")
                        .WithMany()
                        .HasForeignKey("acctno")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.Courier", "Courier")
                        .WithMany("PickupRequests")
                        .HasForeignKey("couriercode")
                        .HasPrincipalKey("couriercode")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.District", "District")
                        .WithMany()
                        .HasForeignKey("distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("Branch");

                    b.Navigation("Courier");

                    b.Navigation("District");
                });

            modelBuilder.Entity("iDss.X.Models.PickupSchedule", b =>
                {
                    b.HasOne("iDss.X.Models.PickupRegular", "PickupRegular")
                        .WithMany()
                        .HasForeignKey("puregid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("PickupRegular");
                });

            modelBuilder.Entity("iDss.X.Models.PickupStatusPool", b =>
                {
                    b.HasOne("iDss.X.Models.PickupRequest", "PickupRequest")
                        .WithMany("PickupStatusPools")
                        .HasForeignKey("pickupno")
                        .HasPrincipalKey("pickupno")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PickupRequest");
                });

            modelBuilder.Entity("iDss.X.Models.PostCode", b =>
                {
                    b.HasOne("iDss.X.Models.Village", "Village")
                        .WithMany()
                        .HasForeignKey("villid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Village");
                });

            modelBuilder.Entity("iDss.X.Models.ReassignPickup", b =>
                {
                    b.HasOne("iDss.X.Models.PickupRequest", "PickupRequest")
                        .WithMany("ReassignPickups")
                        .HasForeignKey("pickupno")
                        .HasPrincipalKey("pickupno")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PickupRequest");
                });

            modelBuilder.Entity("iDss.X.Models.ReschedulePickup", b =>
                {
                    b.HasOne("iDss.X.Models.PickupRequest", "PickupRequest")
                        .WithMany("ReschedulePickups")
                        .HasForeignKey("pickupno")
                        .HasPrincipalKey("pickupno")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PickupRequest");
                });

            modelBuilder.Entity("iDss.X.Models.ShipmentDetail", b =>
                {
                    b.HasOne("iDss.X.Models.PickupRequest", "PickupRequest")
                        .WithMany("ShipmentDetails")
                        .HasForeignKey("pickupno")
                        .HasPrincipalKey("pickupno")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("PickupRequest");
                });

            modelBuilder.Entity("iDss.X.Models.ShipperDetail", b =>
                {
                    b.HasOne("iDss.X.Models.Account", "Account")
                        .WithMany()
                        .HasForeignKey("acctno")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.ShipmentDetail", "ShipmentDetail")
                        .WithOne("ShipperDetail")
                        .HasForeignKey("iDss.X.Models.ShipperDetail", "awb")
                        .HasPrincipalKey("iDss.X.Models.ShipmentDetail", "awb")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.District", "District")
                        .WithMany()
                        .HasForeignKey("distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("Branch");

                    b.Navigation("District");

                    b.Navigation("ShipmentDetail");
                });

            modelBuilder.Entity("iDss.X.Models.SuccessPickup", b =>
                {
                    b.HasOne("iDss.X.Models.PickupRequest", "PickupRequest")
                        .WithMany("SuccessPickups")
                        .HasForeignKey("pickupno")
                        .HasPrincipalKey("pickupno")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PickupRequest");
                });

            modelBuilder.Entity("iDss.X.Models.TrxStaging", b =>
                {
                    b.HasOne("iDss.X.Models.Account", "Account")
                        .WithMany()
                        .HasForeignKey("acctno")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.Branch", "BranchCne")
                        .WithMany()
                        .HasForeignKey("branchcne")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.Branch", "BranchOri")
                        .WithMany()
                        .HasForeignKey("branchori")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.District", "DistrictCne")
                        .WithMany()
                        .HasForeignKey("c_distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.PickupRequest", "PickupRequest")
                        .WithMany("TrxStagings")
                        .HasForeignKey("pickupno")
                        .HasPrincipalKey("pickupno")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("iDss.X.Models.District", "DistrictOri")
                        .WithMany()
                        .HasForeignKey("s_distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("BranchCne");

                    b.Navigation("BranchOri");

                    b.Navigation("DistrictCne");

                    b.Navigation("DistrictOri");

                    b.Navigation("PickupRequest");
                });

            modelBuilder.Entity("iDss.X.Models.UnsuccessPickup", b =>
                {
                    b.HasOne("iDss.X.Models.PickupRequest", "PickupRequest")
                        .WithMany("UnsuccessPickups")
                        .HasForeignKey("pickupno")
                        .HasPrincipalKey("pickupno")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("PickupRequest");
                });

            modelBuilder.Entity("iDss.X.Models.Village", b =>
                {
                    b.HasOne("iDss.X.Models.District", "District")
                        .WithMany()
                        .HasForeignKey("distid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("District");
                });

            modelBuilder.Entity("iDss.X.Models.VoidTransaction", b =>
                {
                    b.HasOne("iDss.X.Models.ShipmentDetail", "ShipmentDetail")
                        .WithOne("VoidTransaction")
                        .HasForeignKey("iDss.X.Models.VoidTransaction", "awb")
                        .HasPrincipalKey("iDss.X.Models.ShipmentDetail", "awb")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("iDss.X.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("branchid")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Branch");

                    b.Navigation("ShipmentDetail");
                });

            modelBuilder.Entity("iDss.X.Models.Account", b =>
                {
                    b.Navigation("AccountAddrs");

                    b.Navigation("AccountCros");
                });

            modelBuilder.Entity("iDss.X.Models.AppModule", b =>
                {
                    b.Navigation("Menus");
                });

            modelBuilder.Entity("iDss.X.Models.AppModuleCtg", b =>
                {
                    b.Navigation("Modules");
                });

            modelBuilder.Entity("iDss.X.Models.Branch", b =>
                {
                    b.Navigation("couriers");
                });

            modelBuilder.Entity("iDss.X.Models.CRO", b =>
                {
                    b.Navigation("AccountCros");
                });

            modelBuilder.Entity("iDss.X.Models.Courier", b =>
                {
                    b.Navigation("CheckpointPools");

                    b.Navigation("PickupRegulars");

                    b.Navigation("PickupRequests");
                });

            modelBuilder.Entity("iDss.X.Models.Industry", b =>
                {
                    b.Navigation("CIFs");
                });

            modelBuilder.Entity("iDss.X.Models.PackingSize", b =>
                {
                    b.Navigation("PackingPrices");
                });

            modelBuilder.Entity("iDss.X.Models.PackingType", b =>
                {
                    b.Navigation("PackingPrices");
                });

            modelBuilder.Entity("iDss.X.Models.PickupRequest", b =>
                {
                    b.Navigation("CancelPickups");

                    b.Navigation("PickupStatusPools");

                    b.Navigation("ReassignPickups");

                    b.Navigation("ReschedulePickups");

                    b.Navigation("ShipmentDetails");

                    b.Navigation("SuccessPickups");

                    b.Navigation("TrxStagings");

                    b.Navigation("UnsuccessPickups");
                });

            modelBuilder.Entity("iDss.X.Models.ShipmentDetail", b =>
                {
                    b.Navigation("ConsigneeDetail")
                        .IsRequired();

                    b.Navigation("PaymentDetail")
                        .IsRequired();

                    b.Navigation("ShipperDetail")
                        .IsRequired();

                    b.Navigation("VoidTransaction")
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
